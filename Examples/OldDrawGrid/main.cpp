...
void
svg_line(std::ostream &file, double x1, double y1, double z1, double x2, double y2, double z2, double modelview[16], double projection[16], int viewport[4]);
...
public:
    ...
    coord operator*(const coord &other) const {
        return coord(p[1] * other.p[2] - p[2] * other.p[1], p[2] * other.p[0] - p[0] * other.p[2], p[0] * other.p[1] - p[1] * other.p[0]);
    }
...
Storage::real GetSize(Cell c) {
    Storage::real bounds[3][2] = {{1.0e20, -1.0e20},
                                  {1.0e20, -1.0e20},
                                  {1.0e20, -1.0e20}};
...
public:
    ...
    Streamline(const Octree &octsearch, coord pos, Tag velocity_tag, Tag cell_size, Storage::real velocity_min, Storage::real velocity_max, Storage::real sign,
               MarkerType visited) {
    ...
    Streamline(const Streamline &other) {
        points = other.points;
        velarr = other.velarr;
    }

    Streamline &operator=(Streamline const &other) {
        points = other.points;
        velarr = other.velarr;
        return *this;
    }
...
void PrintTags(Mesh *m, ElementType etypes) {
    std::cout << std::setw(name_width) << "Name" << std::setw(type_width) << "Type" << std::setw(elems_width) << "Element" << std::setw(sparse_width)
              << "Sparse" << std::setw(length_width) << "Length" << std::endl;
    for (Mesh::iteratorTag t = m->BeginTag(); t != m->EndTag(); ++t) {
        bool print = false;
        for (ElementType etype = NODE; etype <= MESH; etype = etype << 1)
            if ((etype & etypes) && t->isDefined(etype)) {
                print = true;
                break;
            }
        if (print) PrintTag(*t);
...
public:
    ...
    void DrawSVG(std::ostream &file, double modelview[16], double projection[16], int viewport[4]) {
        ...
        for (int i = 0; i < ticks.size() - 1; ++i) {
            colors[i].set_color();
            gluProject(left, bottom + ticks[i] * (top - bottom), 0, modelview, projection, viewport, &px1, &py1, &z);
            py1 = height - py1;
            gluProject(right, bottom + ticks[i + 1] * (top - bottom), 0, modelview, projection, viewport, &px2, &py2, &z);
            py2 = height - py2;
            file << "<def>" << std::endl;
            file << "<linearGradient id=\"grad" << i << "\" gradientUnits=\"userSpaceOnUse\" x1=\"" << px1 << "\" y1=\"" << py1 << "\" x2=\"" << px1
                 << "\" y2=\"" << py2 << "\">" << std::endl;
            file << "<stop offset=\"0%\" stop-color=\"rgb(" << floor(colors[i].r() * 255) << "," << floor(colors[i].g() * 255) << ","
                 << floor(colors[i].b() * 255) << ")\"/>" << std::endl;
            file << "<stop offset=\"100%\" stop-color=\"rgb(" << floor(colors[i + 1].r() * 255) << "," << floor(colors[i + 1].g() * 255) << ","
                 << floor(colors[i + 1].b() * 255) << ")\"/>" << std::endl;
            file << "</linearGradient>" << std::endl;
            file << "</def>" << std::endl;
            file << "<rect stroke=\"none\" x=\"" << px1 << "\" y=\"" << py2 << "\" width=\"" << px2 - px1 << "\" height=\"" << py1 - py2
                 << "\" fill=\"url(#grad" << i << ")\"/>" << std::endl;
        }
        ...
        for (int i = 0; i < tickmarks; ++i) {
            float t = 1.0f * i / static_cast<float>(tickmarks - 1);
            glRasterPos2f(text_pos, bottom + t * (top - bottom));
            gluProject(text_pos, bottom + t * (top - bottom), 0, modelview, projection, viewport, &px, &py, &z);
            py = height - py;
            file << "<text x=\"" << px << "\" y=\"" << py << "\">" << min + t * (max - min) << "</text>" << std::endl;
        }
        if (comment != "") {
            gluProject(left, bottom - 0.04, 0, modelview, projection, viewport, &px, &py, &z);
            py = height - py;
            file << "<text x=\"" << px << "\" y=\"" << py << "\">" << comment.c_str() << "</text>" << std::endl;
        }

        gluProject(left, bottom, 0, modelview, projection, viewport, &px1, &py1, &z);
        py1 = height - py1;
        gluProject(right, top, 0, modelview, projection, viewport, &px2, &py2, &z);
        py2 = height - py2;
        file << "<rect stroke=\"black\" fill=\"none\" x=\"" << px1 << "\" y=\"" << py2 << "\" width=\"" << px2 - px1 << "\" height=\"" << py1 - py2 << "\"/>"
             << std::endl;
...
public:
    static void radix_sort_dist(std::vector<face2gl> &set) {
        ...
        }
        {
            ...
            for (i = 0; i < kHist; i++) {
                b0[kHist - 1] = b0[i] + sum0;
                b0[i] = sum0 - 1;
                sum0 = b0[kHist - 1];
                b1[kHist - 1] = b1[i] + sum1;
                b1[i] = sum1 - 1;
                sum1 = b1[kHist - 1];
                b2[kHist - 1] = b2[i] + sum2;
                b2[i] = sum2 - 1;
                sum2 = b2[kHist - 1];
            }
        }
    ...
    face2gl() : verts(), colors(), texcoords() {
        etype = NONE;
        id = 0;
        dist = 0;
        flag = false;
    ...
    void svg_draw_colour(std::ostream &file, double modelview[16], double projection[16], int viewport[4]) const {
        ...
        if (colors.empty() && texcoords.empty()) {
            ...
            gluProject(cnt[0], cnt[1], cnt[2], modelview, projection, viewport, &pcntx, &pcnty, &z);
            pcnty = height - pcnty;
            ...
            for (unsigned k = 0; k < verts.size(); k += 3) {
                gluProject(verts[k + 0], verts[k + 1], verts[k + 2], modelview, projection, viewport, &pverts1x, &pverts1y, &z);
                pverts1y = height - pverts1y;
                gluProject(verts[(k + 3) % verts.size() + 0], verts[(k + 3) % verts.size() + 1], verts[(k + 3) % verts.size() + 2], modelview, projection,
                           viewport, &pverts2x, &pverts2y, &z);
                pverts2y = height - pverts2y;
                file << "<polygon points=\"" << pcntx << "," << pcnty << " " << pverts1x << "," << pverts1y << " " << pverts2x << "," << pverts2y << "\"/>"
                     << std::endl;
            }
            file << "</g>" << std::endl;
            ...
            gluProject(cnt[0], cnt[1], cnt[2], modelview, projection, viewport, &pcntx, &pcnty, &z);
            pcnty = height - pcnty;
            file << "<g stroke=\"none\" fill=\"rgb(" << floor(cntcolor.r() * 255) << "," << floor(cntcolor.g() * 255) << "," << floor(cntcolor.b() * 255)
                 << ")\">" << std::endl;
            for (unsigned k = 0; k < verts.size(); k += 3) {
                gluProject(verts[k + 0], verts[k + 1], verts[k + 2], modelview, projection, viewport, &pverts1x, &pverts1y, &z);
                pverts1y = height - pverts1y;
		...
		*/
        ...
        if (::drawedges && ::drawedges != 2) {
            ...
            for (unsigned k = 0; k < verts.size(); k += 3) {
                gluProject(verts[k % verts.size() + 0], verts[k % verts.size() + 1], verts[k % verts.size() + 2], modelview, projection, viewport, &px, &py,
                           &z);
                py = height - py;
                file << px << "," << py << " ";
            }
    ...
    void svg_draw(std::ostream &file, double modelview[16], double projection[16], int viewport[4]) const {
        ...
        for (unsigned k = 0; k < verts.size(); k += 3) {
            gluProject(verts[k + 0], verts[k + 1], verts[k + 2], modelview, projection, viewport, &pverts1x, &pverts1y, &z);
            pverts1y = height - pverts1y;
            gluProject(verts[(k + 3) % verts.size() + 0], verts[(k + 3) % verts.size() + 1], verts[(k + 3) % verts.size() + 2], modelview, projection, viewport,
                       &pverts2x, &pverts2y, &z);
            pverts2y = height - pverts2y;
            file << "<polygon points=\"" << pcntx << "," << pcnty << " " << pverts1x << "," << pverts1y << " " << pverts2x << "," << pverts2y << "\"/>"
                 << std::endl;
    ...
    void svg_drawedges(std::ostream &file, double modelview[16], double projection[16], int viewport[4]) const {
        ...
        file << "<polyline fill=\"none\" points=\"";
        for (unsigned k = 0; k < verts.size(); k += 3) {
            gluProject(verts[k % verts.size() + 0], verts[k % verts.size() + 1], verts[k % verts.size() + 2], modelview, projection, viewport, &px, &py, &z);
            py = height - py;
            file << px << "," << py << " ";
        }
        file << "\" />" << std::endl;
    ...
    void set_elem(ElementType _etype, Storage::integer _id) {
        etype = _etype;
        id = _id;
    }
    ...
    volumetric(Mesh *_m) {
        ...
        for (Mesh::iteratorCell it = m->BeginCell(); it != m->EndCell(); ++it) {
            ...
            for (ElementArray<Face>::iterator f = faces.begin(); f != faces.end(); ++f) {
                f->Centroid(cntf);
                Storage::real d = sqrt(
                        (cnt[0] - cntf[0]) * (cnt[0] - cntf[0]) + (cnt[1] - cntf[1]) * (cnt[1] - cntf[1]) + (cnt[2] - cntf[2]) * (cnt[2] - cntf[2]));
        ...
        if (interactive) {
            for (int k = 0; k < points.size(); ++k)
                if (k % 100 == 0) {
                    ...
                    glVertex3f(points[k].coords[0] + (right[0] * rmult + up[0]) * points[k].diam * mult,
                               points[k].coords[1] + (right[1] * rmult + up[1]) * points[k].diam * mult,
                               points[k].coords[2] + (right[2] * rmult + up[2]) * points[k].diam * mult);
                    glVertex3f(points[k].coords[0] + (right[0] * rmult * 2) * points[k].diam * mult,
                               points[k].coords[1] + (right[1] * rmult * 2) * points[k].diam * mult,
                               points[k].coords[2] + (right[2] * rmult * 2) * points[k].diam * mult);
                    glVertex3f(points[k].coords[0] + (right[0] * rmult - up[0]) * points[k].diam * mult,
                               points[k].coords[1] + (right[1] * rmult - up[1]) * points[k].diam * mult,
                               points[k].coords[2] + (right[2] * rmult - up[2]) * points[k].diam * mult);
                    glVertex3f(points[k].coords[0] + (-right[0] * rmult - up[0]) * points[k].diam * mult,
            ...
            for (int k = 0; k < points.size(); ++k) {
                ...
                glBegin(GL_TRIANGLE_FAN);
                glVertex3f(points[k].coords[0], points[k].coords[1], points[k].coords[2]);
                glVertex3f(points[k].coords[0] + (right[0] + up[0]) * points[k].diam * mult, points[k].coords[1] + (right[1] + up[1]) * points[k].diam * mult,
                           points[k].coords[2] + (right[2] + up[2]) * points[k].diam * mult);
                glVertex3f(points[k].coords[0] + (right[0] * 2) * points[k].diam * mult, points[k].coords[1] + (right[1] * 2) * points[k].diam * mult,
                           points[k].coords[2] + (right[2] * 2) * points[k].diam * mult);
                glVertex3f(points[k].coords[0] + (right[0] - up[0]) * points[k].diam * mult, points[k].coords[1] + (right[1] - up[1]) * points[k].diam * mult,
                           points[k].coords[2] + (right[2] - up[2]) * points[k].diam * mult);
                glVertex3f(points[k].coords[0] + (-right[0] - up[0]) * points[k].diam * mult, points[k].coords[1] + (-right[1] - up[1]) * points[k].diam * mult,
                           points[k].coords[2] + (-right[2] - up[2]) * points[k].diam * mult);
...
class kdtree {
    ...
    void kdtree_build(int dim, int &done, int total, struct entry *temp) {
        if (size > 1) {
            if (size > 128) radix_sort(dim, temp);
            else
                switch (dim) {
                    case 0:
                        qsort(set, size, sizeof(entry), cmpElements0);
                        break;
                    case 1:
                        qsort(set, size, sizeof(entry), cmpElements1);
                        break;
                    case 2:
                        qsort(set, size, sizeof(entry), cmpElements2);
                        break;
                }
    ...
    bool sub_intersect_plane_edge(Tag clip_point, Tag clip_state, ElementArray<Cell> &cells, MarkerType mrk, double p[3], double n[3]) {
        if (size == 1) {
            ...
            if (clip) {
                ...
                for (INMOST_DATA_ENUM_TYPE k = 0; k < ecells.size(); ++k)
                    if (!ecells[k].GetMarker(mrk)) {
    ...
    void unmark_old_edges(Tag clip_state) {
        if (size == 1) {
            ...
            if (children[0].marked) {
                children[0].unmark_old_edges(clip_state);
                marked = 0;
            }
            if (children[1].marked) {
                children[1].unmark_old_edges(clip_state);
                marked = 0;
            }
        }
    }
...
public:
    ...
    void clip_plane(Storage::real p[3], Storage::real n[3]) {
        ...
        for (INMOST_DATA_ENUM_TYPE k = 0; k < cells.size(); ++k) {
                ...
                for (INMOST_DATA_ENUM_TYPE r = 0; r < edges.size(); ++r) {
                    ...
                    if (state == CLIP_FULL) {
                        ...
                        edge_point n1 = edge_point(&edges[r].getBeg()->Coords()[0], ntotedges,
                                                   visualization_tag.isValid() ? edges[r].getBeg()->RealDF(visualization_tag) : 0.f);
                        edge_point n2 = edge_point(&edges[r].getEnd()->Coords()[0], ntotedges,
                                                   visualization_tag.isValid() ? edges[r].getEnd()->RealDF(visualization_tag) : 0.f);
                        ...
                        edge_point n = edge_point(&edges[r].RealArrayDF(clip_point)[0], ntotedges,
                                                  compute_value(edges[r], &edges[r].RealArrayDF(clip_point)[0]));
                        bool add = true;
            ...
            if (full_face.isValid()) {
                ...
                while (havestart) {
                    havestart = false;
                    for (int r = 0; r < ntotedges && !havestart; ++r)
                        if (!closed[r]) {
    ...
    void draw_clip(INMOST_DATA_ENUM_TYPE pace, Storage::real n[3]) {
        ...
        glBegin(GL_TRIANGLES);
        for (INMOST_DATA_ENUM_TYPE k = 0; k < cells.size(); k += pace)
            if (cells[k]->GetMarker(marker)) {
                Storage::real_array cl = cells[k]->RealArray(clips);
                Storage::real_array clv = cells[k]->RealArray(clipsv);
                Storage::integer_array cln = cells[k]->IntegerArray(clipsn);
                    ...
                    if (cells[k]->GetMarker(marker)) {
                        ...
                        for (int r = 0; r < cln.size(); ++r) {
                            ...
                            for (INMOST_DATA_ENUM_TYPE q = offset; q < offset + cln[r]; q++) {
                                ...
                                pos[2] = cl[q * 3 + 2] + t * n[2];
                                glVertex3dv(pos);
                                t = (clv[(q + 1 - offset) % cln[r] + offset] - CommonColorBar->get_min()) /
                                    (CommonColorBar->get_max() - CommonColorBar->get_min());
...
public:
    Input(int *val, std::string comment) : comment(comment) {
        input_link = val;
        type = Integer;
        canceled = false;
        done = false;
    ...
    Input(double *val, std::string comment) : comment(comment) {
        input_link = val;
        type = Double;
        canceled = false;
        done = false;
    ...
    Input(char *val, std::string comment) : comment(comment) {
        input_link = val;
        type = String;
        canceled = false;
        done = false;
    ...
    Input(void *link, InputType type, std::string comment) : comment(comment), input_link(link), type(type) {
        canceled = false;
        done = false;
        str = "";
    }

    Input(const Input &other) : str(other.str), comment(other.comment), input_link(other.input_link), type(other.type), done(other.done),
    ...
    void KeyPress(char c) {
        if (c == 13) {
            ...
            glutPostRedisplay();
        } else if (type == String || ((c >= '0' && c <= '9') || ((str.empty() || tolower(*str.rbegin()) == 'e') && (c == '+' || c == '-')) ||
                                      (type == Double && (c == '.' || c == 'e' || c == 'E')))) {
            str += c;
            glutPostRedisplay();
        }
    }
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (glutGetModifiers() & (GLUT_ACTIVE_CTRL))
        std::cout << "pressed " << ((char) (key)) << " int " << ((int) key) << " ctrl " << (glutGetModifiers() & GLUT_ACTIVE_CTRL ? "yes" : "no") << " shift "
                  << (glutGetModifiers() & GLUT_ACTIVE_SHIFT ? "yes" : "no") << " alt " << (glutGetModifiers() & GLUT_ACTIVE_ALT ? "yes" : "no") << std::endl;
    if (CommonInput != NULL) {
...
void keyboard2(unsigned char key, int x, int y) {
    (void) x;
    (void) y;
    if (key == '=' || key == '+' || key == '_' || key == '-' || key == 'w' || key == 's' || key == 'a' || key == 'd' || key == 'r' || key == 'p' ||
        key == 'z') {
        //printf("depressed\n");
        interactive = false;
        glutPostRedisplay();
...
double display_elem_info(Element e, double top, double left, double interval) {
    ...
    for (Mesh::iteratorTag t = mesh->BeginTag(); t != mesh->EndTag(); ++t)
        if (t->isDefined(e->GetElementType()))
            if (e->HaveData(*t))
                bottom -= interval;
...

    {
        ...
        if (oclipper) {
            ...
            if (interactive && clipboxupdate) {
                ...
                INMOST_DATA_ENUM_TYPE opace = !planecontrol ? std::max<INMOST_DATA_ENUM_TYPE>(1, std::min<INMOST_DATA_ENUM_TYPE>(15, oclipper->size() / 100))
                                                            : 1;
                                    ...
                                    if (visualization_type != NONE) {
                                        if (source_tag.isDefined(visualization_type)) {
                                            ...
                                            for (Mesh::iteratorNode it = mesh->BeginNode(); it != mesh->EndNode(); ++it) {
                                                ...
                                                for (ElementArray<Element>::iterator jt = elems.begin(); jt != elems.end(); ++jt)
                                                    if (jt->HaveData(source_tag) && jt->GetDataSize(source_tag) > comp) {
                                                        jt->Centroid(cnt);
                                                        dist = (cnt[0] - coords[0]) * (cnt[0] - coords[0]) + (cnt[1] - coords[1]) * (cnt[1] - coords[1]) +
                                                               (cnt[2] - coords[2]) * (cnt[2] - coords[2]);
                                                        wgt = 1.0 / (dist + 1.0e-8);
                                                        val += wgt * (source_tag.GetDataType() == DATA_REAL ? jt->RealArray(source_tag)[comp]
                                                                                                            : static_cast<Storage::real>(jt->IntegerArray(
                                                                        source_tag)[comp]));
                                                        vol += wgt;
                                                    }
                                                res = val / vol;
...
svg_line(std::ostream &file, double x1, double y1, double z1, double x2, double y2, double z2, double modelview[16], double projection[16], int viewport[4]) {
    double px1, py1, z;
    double px2, py2;
    gluProject(x1, y1, z1, modelview, projection, viewport, &px1, &py1, &z);
    py1 = height - py1;
    gluProject(x2, y2, z2, modelview, projection, viewport, &px2, &py2, &z);
    py2 = height - py2;
	...
	*/
    ...
    if (mesh->HaveTag("ADDED_ELEMENTS")) {
        ...
        for (Mesh::iteratorFace it = mesh->BeginFace(); it != mesh->EndFace(); ++it)
            if (it->Integer(add) && !it->Boundary())
                added_faces.push_back(DrawFace(it->self()));
    }
    ...
    if (mesh->HaveTag("DUAL_HARMONIC_POINT")) {
        ...
        for (Mesh::iteratorElement it = mesh->BeginElement(CELL | FACE); it != mesh->EndElement(); ++it)
            if (it->HaveData(h2)) {
