...
#pragma omp parallel for
        ...
        for (Storage::integer fi = 0; fi < m->FaceLastLocalID(); ++fi)
            if (m->isValidElement(FACE, fi)) {
                ...
                if (((!r1->isValid() || (s1 = r1->GetStatus()) == Element::Ghost) ? 0 : 1) +
                    ((!r2->isValid() || (s2 = r2->GetStatus()) == Element::Ghost) ? 0 : 1) == 0)
                    continue;
...
#pragma omp parallel for
#endif
        for (Storage::integer ci = 0; ci < m->CellLastLocalID(); ++ci)
            if (m->isValidElement(CELL, ci)) {
