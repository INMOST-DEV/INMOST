...
std::string problem_name = "enright_sphere";

real max[3] = {-1.0e20, -1.0e20, -1.0e20}, min[3] = {1.0e20,1.0e20,1.0e20};
real disc_cx = 0.5;
real disc_cy = 0.25;
real disc_rad = 0.2;//0.15;
...
int main(int argc, char ** argv)
{
	...
	for(Mesh::iteratorFace it = m->BeginFace(); it != m->EndFace(); ++it)
	{
		...
		//real w = -sin(2*pi*x)*sin(2*pi*y)*sin(pi*z)*sin(pi*z); 

		//std::cout << "xyz: " << x << "," << y << "," << z << std::endl;
		//std::cout << "uvw: " << u << "," << v << "," << w << std::endl;

		//fill in normal component of the velocity
		it->Real(velocity) = nrm[0]*u + nrm[1]*v;// + nrm[2]*w;
		
		if( it->Boundary() )
		{
			real_array bc = it->RealArray(bndcond); //create bc
			//dirichlet bc with concentration 0
			bc[0] = 1;
			bc[1] = 0;
			bc[2] = 0;
		}
	...
	if( argc > 3 )
	{
		...
		time_info[0] = dt;
		std::cout << "Selected time step: " << dt << " based on CFL, Vmax = " << Vmax << std::endl;
	}
	else cfl = Vmax*dt/mesh_radius;
	std::cout << "CFL: " << cfl << std::endl;

	for(Mesh::iteratorCell it = m->BeginCell(); it != m->EndCell(); ++it)
		it->Real(conc) = it->Mean(enright_shape,1.0); //initial concentration
