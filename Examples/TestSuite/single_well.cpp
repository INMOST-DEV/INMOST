...
void prepare_tensor(Storage::real t[9], Storage::real ratio)
{
  ...
  Storage::real anglex = 30, angley = 45, anglez = 60;
...
int main(int argc, char *argv[]) 
{
  ...
  if( cut_grid )
  {
    for (Mesh::iteratorCell it = mesh->BeginCell(); it != mesh->EndCell(); ++it)
    {
      ...
      if( is2d )
      {
        if( cnt[0] > 4.0/9.0 && cnt[0] < 5.0/9.0 && cnt[1] > 4.0/9.0 && cnt[1] < 5.0/9.0 )
          it->Delete();
      }
      else
      {
  ...
  for(Mesh::iteratorElement it = mesh->BeginElement(FACE|NODE); it != mesh->EndElement(); ++it) if( it->Boundary() )
  {
    ...
    if( ( is2d && (cnt[0] > 4.0/9.0-eps && cnt[0] < 5.0/9.0+eps && cnt[1] > 4.0/9.0-eps && cnt[1] < 5.0/9.0+eps)) ||
        (!is2d && (cnt[0] > 0.4-eps && cnt[0] < 0.6+eps && cnt[1] > 0.4-eps && cnt[1] < 0.6+eps && cnt[2] > 0.4-eps && cnt[2] < 0.6+eps)))
    {
        Storage::real_array bnd = it->RealArray(bndcond);
        bnd[0] = 1.0; //dirichlet
        bnd[1] = 0.0;
		bnd[2] = inner_boundary_pressure;
        if( it->GetElementType() == FACE ) numinner++;
        if( it->GetElementType() == NODE ) numinnern++;
    }
    else if( (cnt[0] < eps || cnt[0] > 1.0-eps || cnt[1] < eps || cnt[1] > 1 - eps) || (!is2d && (cnt[2] < 1-eps || cnt[2] > 1 - eps)) )
    {
        //std::cout << ElementTypeName(it->GetElementType()) << " " << cnt[0] << " " << cnt[1] << " " << cnt[2] << " " << (cnt[0] < eps || cnt[0] > 1.0-eps || cnt[1] < eps || cnt[1] > eps) << std::endl;
        Storage::real_array bnd = it->RealArray(bndcond);
        bnd[0] = 1.0; //dirichlet
        bnd[1] = 0.0;
	    bnd[2] = outer_boundary_pressure;
  ...
  Storage::real mat[9];
  if( !is2d ) prepare_tensor(mat,ratio);
  else prepare_tensor_2d(mat,ratio);
