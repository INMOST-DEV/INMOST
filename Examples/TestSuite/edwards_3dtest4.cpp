...
std::string problem_name = "edwards_3dtest4";
...
int main(int argc, char ** argv)
{
	...
	for(Mesh::iteratorElement it = m->BeginElement(CELL|FACE|EDGE|NODE); it != m->EndElement(); ++it)
	{
		...
		if( x < 0.5 )
		{
			sol = (2*x-1)*(2*x-1) + 5*(2*x-1)*y;// + 5*(2*x-1)*z;
			K(0,0) = K(1,1) = K(2,2) = 1;
		}
		else
		{
			sol = (2*x-1)*(2*x-1) + (2*x-1)*y;// + (2*x-1)*z;
			K(0,0) = K(1,1) = K(2,2) = 5;
			K(0,1) = K(1,0) = K(0,2) = K(2,0) = K(1,2) = K(2,1) = 4;
			K(0,2) = K(2,0) = K(1,2) = K(2,1) = 0;
		}
		...
		*/
		...
		if( it->GetElementType() == CELL )
		{
			...
			it->RealArray(rvel)[0] = get_value(Kgrad(0,0));
			it->RealArray(rvel)[1] = get_value(Kgrad(1,0));
			it->RealArray(rvel)[2] = get_value(Kgrad(2,0));
		}
