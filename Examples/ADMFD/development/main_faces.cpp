      ...
      do
      {
        ...
        for( int q = 0; q < m->CellLastLocalID(); ++q ) if( m->isValidCell(q) ) //loop over cells
        {
          ...
          vMatrix DIV = -(D*GRAD).Transpose()*M; //cell-wise div
           ...
           {
              rMatrix check = GRAD.Transpose()*M*GRAD, U,S,V;
              check.SVD(U,S,V);
              int rank = 0;
              for(int k = 0; k < S.Rows(); ++k)
                  if( fabs(S(k,k)) > 1.0e-5 )
                      rank++;
              if( rank != NF ) std::cout << "Matrix is not full rank: " << rank << " should be " << NF << std::endl;
          }
          /*
          std::cout << "D" << std::endl;
          D.Print();
        ...
        iter++;
      } while( R.Norm() > 1.0e-4 && iter < 10 ); //check the residual norm
    } 
