...
{
	//shortcuts
	__INLINE HandleType & hParent(Element::adj_type & arr) {return arr[0];}
	__INLINE HandleType & hSibling(Element::adj_type & arr) {return arr[1];}
	__INLINE HandleType & hChild(Element::adj_type & arr) {return arr[2];}
	__INLINE HandleType & hSorted(Element::adj_type & arr) {return arr[3];}
	...
	ElementArray<Node> ElementSet::getNodes() const
	{
		...
		{
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)//check for type filters invalid handles
				if( GetHandleElementType(lc[it]) & NODE )
					ret.push_back(lc[it]);
		}
		return ret;
	}
	...
	ElementArray<Edge> ElementSet::getEdges() const
	{
		...
		if( m->isMeshModified() )
		{
			MarkerType hm = m->HideMarker();
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)
				if( (GetHandleElementType(lc[it]) & EDGE) && !m->GetMarker(lc[it],hm) )//check for type filters invalid handles
					ret.push_back(lc[it]);
		}
		else
		{
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)//check for type filters invalid handles
				if( GetHandleElementType(lc[it]) & EDGE )
					ret.push_back(lc[it]);
		}
		return ret;
	}
	...
	ElementArray<Face> ElementSet::getFaces() const
	{
		...
		if( m->isMeshModified() )
		{
			MarkerType hm = m->HideMarker();
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)
				if( (GetHandleElementType(lc[it]) & FACE) && !m->GetMarker(lc[it],hm) )//check for type filters invalid handles
					ret.push_back(lc[it]);
		}
		else
		{
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)//check for type filters invalid handles
				if( GetHandleElementType(lc[it]) & FACE )
					ret.push_back(lc[it]);
		}
		return ret;
	}
	...
	ElementArray<Cell> ElementSet::getCells() const
	{
		...
		if( m->isMeshModified() )
		{
			MarkerType hm = m->HideMarker();
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)
				if( (GetHandleElementType(lc[it]) & CELL) && !m->GetMarker(lc[it],hm) )//check for type filters invalid handles
					ret.push_back(lc[it]);
		}
		else
		{
			for(Element::adj_type::size_type it = 0; it < lc.size(); ++it)//check for type filters invalid handles
				if( GetHandleElementType(lc[it]) & CELL )
					ret.push_back(lc[it]);
		}
		return ret;
	}
		...
		case UNSORTED_COMPARATOR:
			{
				...
				while( it != num )
				{
					if( !m->GetMarker(h[it],mrk) )
					{
						...
						else lc.push_back(hc[it]);
					}
					++it;
				}
				RemMarkerElements(mrk);
				m->ReleaseMarker(mrk);
			}
			break;
		case HANDLE_COMPARATOR:
			{
				...
				while(it != iend && jt != jend)
				{
					...
					else {mitprev = mit; *mit = *jt; ++mit; ++it;}
				}
				//attach rest
				...
				MarkerType mrk = m->CreateMarker();
				m->SetMarkerArray(merge.data(),hSorted(hc),mrk);
				while(it != lc.end() && *it != InvalidHandle() && !m->GetMarker(*it,mrk) )
				{
					m->SetMarker(*it,mrk);
					*mit++ = *it++;
				}
				merge.resize(static_cast<Element::adj_type::size_type>(mit-merge.begin()));
				m->RemMarkerArray(merge.data(),static_cast<enumerator>(merge.size()),mrk);
				m->ReleaseMarker(mrk);
		...
		case GLOBALID_COMPARATOR:
			{
				...
				while(it != iend && jt != jend)
				{
					if( *it == InvalidHandle() || m->GetMarker(*it,mrk) ) ++it;
					else if( *jt == InvalidHandle() || m->GetMarker(*jt,mrk) ) ++jt;
					else if( Mesh::GlobalIDComparator(m)(*it,*jt) ) {*mit = *it; ++mit; ++it;}
					else {*mit = *jt; ++mit; ++it;}
				}
				//attach rest
				while(it != iend && *it != InvalidHandle() && !m->GetMarker(*it,mrk) )
				{
					*mit = *it;
					++mit;
					++it;
				}
				while(jt != jend && *jt != InvalidHandle() && !m->GetMarker(*jt,mrk) )
				{
					*mit = *jt;
					++mit;
					++jt;
		...
		case IERARHY_COMPARATOR:
			{
				...
				while(it != iend && jt != jend)
				{
					...
					else if( Mesh::IerarhyComparator(m)(*it,*jt) ) {*mit = *it; ++mit; ++it;}
					else {*mit = *jt; ++mit; ++it;}
				}
				//attach rest
				while(it != iend && *it != InvalidHandle() && !m->GetMarker(*it,mrk) )
		...
		case CENTROID_COMPARATOR:
			{
				...
				while(it != iend && jt != jend)
				{
					...
					else if( Mesh::CentroidComparator(m)(*it,*jt) ) {*mit = *it; ++mit; ++it;}
					else {*mit = *jt; ++mit; ++it;}
				}
				//attach rest
				while(it != iend && *it != InvalidHandle() && !m->GetMarker(*it,mrk) )
	...
	void ElementSet::RemoveElements(const HandleType * h, enumerator num) const
	{
		...
		MarkerType mrk = m->CreateMarker();
		m->SetMarkerArray(h,num,mrk);
		for(i = 0; i < lc.size(); i++)
		{
			if( m->GetMarker(lc[i],mrk) )
			{
				lc[i] = InvalidHandle();
				hc.push_back(i);
			}
		}
		m->RemMarkerArray(h,num,mrk);
		m->ReleaseMarker(mrk);
	}
	...
	ElementArray<Element> ElementSet::Union(const ElementSet & other) const
	{
		...
		{
			if( lc[i] != InvalidHandle() && !m->GetMarker(lc[i],mrk))
			{
				ret.push_back(lc[i]);
	...
	ElementArray<Element> ElementSet::Difference(const HandleType * handles, enumerator num) const
	{
		...
		Element::adj_type & lc = m->LowConn(GetHandle());
		MarkerType mrk = m->CreateMarker();
		m->SetMarkerArray(handles,num,mrk);
		for(Element::adj_type::size_type i = 0; i < lc.size(); ++i)
		{
			if( lc[i] != InvalidHandle() && !m->GetMarker(lc[i],mrk))
			{
		...
		m->RemMarkerArray(handles,num,mrk);
		ret.RemMarker(mrk);
		m->ReleaseMarker(mrk);
		return ret;
	}
	...
	ElementArray<Element> ElementSet::Intersection(const ElementSet & other) const
	{
		...
		ElementArray<Element> ret(m);
		Element::adj_type & lc = m->LowConn(GetHandle());
		MarkerType mrk = m->CreateMarker(), mrk2 = m->CreateMarker();
		other->SetMarkerElements(mrk);
		for(Element::adj_type::size_type i = 0; i < lc.size(); ++i)
		{
			if( lc[i] != InvalidHandle() && m->GetMarker(lc[i],mrk) && !m->GetMarker(lc[i],mrk2))
			{
				ret.push_back(lc[i]);
				m->SetMarker(lc[i],mrk2);
			}
		}
		other->RemMarkerElements(mrk);
		ret.RemMarker(mrk2);
		m->ReleaseMarker(mrk2);
		m->ReleaseMarker(mrk);
		return ret;
	}
		...
		else //other set sorted by the same comparator, just merge them
		{
			...
			while(it != iend && jt != jend)
			{
				...
				else 
				{
					...
					case IERARHY_COMPARATOR: cmp_res = Mesh::IerarhyComparator(m)(*it,*jt); break;
			...
			//attach unsorted part
			while(jt != other_lc.end() && *jt != InvalidHandle() && !m->GetMarker(*jt,mrk) )
			{
				m->SetMarker(*jt,mrk);
				*mit++ = *jt++;
			}
	...
	bool ElementSet::FindHandle(HandleType h, bool use_cmp) const
	{
		...
		else if( cmp == IERARHY_COMPARATOR && use_cmp )
		{
			Element::adj_type::size_type sorted = static_cast<Element::adj_type::size_type>(hSorted(m->HighConn(GetHandle())));
