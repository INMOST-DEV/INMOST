...
#define ADDR(row,col) ((row)*size+(col))
	...
	bool ILUC_preconditioner::Initialize()
	{
		...
		INMOST_DATA_REAL_TYPE mup, mum, NuU = 1, NuL = 1, NuD, NuU_old = 1, NuL_old = 1, NuU_new, NuL_new, vp, vm, v;
...
///////////////////////////////////////////////////////////////////////////////////
					...
					{
						j = LineIndeces[Li];
						if (fabs(LineValues[Li - 1])*NuL*NuU < tau2*Enorm*fabs(LU_Diag[Li-1]) )//tau2)
						{
							LineIndeces[Ui] = j;
							LineIndeces[Li] = UNDEF;
							LineValues[Li - 1] = 0.0;
...
///////////////////////////////////////////////////////////////////////////////////
          ...
          //tol_schur = exp(log(tau2)*0.6+log(tau)*0.4) * Snorm * Snorm / Smax;// * sqrt(Smax*Snum/Snorm);//tau2 * Smax / std::max(NuU,NuL);
					//tol_schur = (Smax-Smin)*(std::min(tau2*sqrt(Smax/Smean),tau)) + Smin;
					//tol_schur = (Smax-Smin)*tau2 + Smin;
					...
					{
#if defined(PRINT_HISTOGRAM)
						//printf("val %g logval %g intlogval %d\n",(fabs(temp[Ui])-Smin)/(Smax-Smin), log((fabs(temp[Ui])-Smin)/(Smax-Smin)), (int)ceil(-log((fabs(temp[Ui])-Smin)/(Smax-Smin))));
						int pos = std::min((int)ceil(-log((fabs(temp[Ui])-Smin)/(Smax-Smin))),31);
						if( pos < 0 ) pos = 31;
						++histogram[ pos ];
						//++histogram[31- (int)floor((fabs(temp[Ui])-Smin)/(Smax-Smin)*31) ];
#endif
						//drop values
          ...
          nzS += S_Address[i].Size();
					...
					fprintf(fschur,"%e; %e; %e; %e; %e; %e; %d; %e; %e; %e; %e; %e\n",
						temp[i],Smin,Smax,Snorm,Smean,Snum,S_Address[i].Size(), NuU,NuL,NuD, tol_schur,Smax / (Snorm/sqrt(Snum)));
#endif
///////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////
				...
				for (i = 0; i < size; i++)
				{
					...
					for (j = i + 1; j < size; j++)
					{
						if (fabs(entries[ADDR(i, j)])*NuU*NuL > tau*LU_Diag[wbeg+i])
						{
					...
					for (j = i + 1; j < size; j++)
					{
						if (fabs(entries[ADDR(j, i)])*NuL*NuU > tau*LU_Diag[wbeg+i] )
						{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void ILUC_preconditioner::Multiply(int level, Solver::Vector & input, Solver::Vector & output)
	{
	...
	void ILUC_preconditioner::ApplyLU(int level, Solver::Vector & input,Solver::Vector & output)
	{
	...
	void ILUC_preconditioner::ApplyB(int level, double alpha, Solver::Vector & x, double beta, Solver::Vector & y) // y = alpha A x + beta y
	{
		INMOST_DATA_ENUM_TYPE k, m, cbeg, cend;
	...
	int ILUC_preconditioner::Descend(int level, Solver::Vector & input, Solver::Vector & output)
	{
		INMOST_DATA_ENUM_TYPE k, m, cbeg, cend;
		//current B block interval
		cbeg = level_interval[level].first;
	...
	int ILUC_preconditioner::Ascend(int level, Solver::Vector & input, Solver::Vector & output)
	{
		level = level - 1;
	...
	bool ILUC_preconditioner::Solve(Solver::Vector & input, Solver::Vector & output)
	{
		assert(&input != &output);
		//
	...
	bool ILUC_preconditioner::ReplaceMAT(Solver::Matrix & A){ if (isInitialized()) Finalize(); Alink = &A; return true; }
	bool ILUC_preconditioner::ReplaceSOL(Solver::Vector & x) {(void)x; return true; }
	bool ILUC_preconditioner::ReplaceRHS(Solver::Vector & b) {(void)b; return true; }
