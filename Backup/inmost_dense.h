...
namespace INMOST
{
	...
	__INLINE typename Promote<VarA, VarB>::type DotProduct(const AbstractMatrixReadOnly<VarA>& A, const AbstractMatrixReadOnly<VarB>& B)
	{
	...
	__INLINE variable DotProduct(const AbstractMatrixReadOnly<INMOST_DATA_REAL_TYPE>& A, const AbstractMatrixReadOnly<variable>& B);
	__INLINE variable DotProduct(const AbstractMatrixReadOnly<variable>& A, const AbstractMatrixReadOnly<INMOST_DATA_REAL_TYPE>& B);
	__INLINE variable DotProduct(const AbstractMatrixReadOnly<variable>& A, const AbstractMatrixReadOnly<variable>& B);
	...
	public:
		...
		AbstractMatrixTranspose<Var> Transpose() const { return AbstractMatrixTranspose<Var>(*this); }
		/// Transpose and conjugate current matrix.
		/// @return Transposed and conjugated matrix.
		AbstractMatrixConjugateTranspose<Var> ConjugateTranspose() const { return AbstractMatrixConjugateTranspose<Var>(*this); }
		/// Conjugate  current matrix.
		/// @return Conjugated matrix.
		AbstractMatrixConjugate<Var> Conjugate() const { return AbstractMatrixConjugate<Var>(*this); }
		...
		MatrixMul<Var, typeB>
			operator*(const AbstractMatrixReadOnly<typeB>& other) const
		{ return MatrixMul<Var, typeB>(*this, other); }
		...
		AbstractMatrixRepack<Var> Repack(enumerator rows, enumerator cols) const {return AbstractMatrixRepack<Var>(*this, rows, cols);}
		...
		/// @return Submatrix of the original matrix.
		AbstractSubMatrix<Var> operator()(enumerator first_row, enumerator last_row, enumerator first_col, enumerator last_col) const
		{ return AbstractSubMatrix<Var>(*this, first_row, last_row, first_col, last_col); }
		...
		/// @return Submatrix of the original matrix.
		AbstractBlockOfMatrix<Var> BlockOf(enumerator nrows, enumerator ncols, enumerator offset_row, enumerator offset_col) const
		{ return AbstractBlockOfMatrix<Var>(*this, nrows, ncols, offset_row, offset_col); }
		...
		/// @return Matrix multiplied by the coefficient.
		template<class A>
		MatrixMulCoef<Var, variable, typename Promote<Var, variable>::type>
		...
		template<class A>
		MatrixDivCoef<Var, variable, typename Promote<Var, variable>::type>
			operator/(shell_expression<A> const& coef) const { return operator/(variable(coef)); }
#endif //USE_AUTODIFF
		...
		AbstractMatrixConcatCols<Var, VarB>
			ConcatCols(const AbstractMatrixReadOnly<VarB>& B) const
		{
			return AbstractMatrixConcatCols<Var, VarB>(*this, B);
		}
		...
		template<typename VarB>
		AbstractMatrixConcatRows<Var,VarB> 
			ConcatRows(const AbstractMatrixReadOnly<VarB>& B) const
		{
			return AbstractMatrixConcatRows<Var, VarB>(*this, B);
		}
		/// Convert into matrix.
	...
	public:
		...
		/// @param j Row index.
		/// @return Reference to constant element.
		__INLINE Var compute(enumerator i, enumerator j) const { return get(i,j); }
		...
		AbstractMatrix & operator-=(const AbstractMatrixReadOnly<typeB> & other)
		{
			...
			for (enumerator i = 0; i < Rows(); ++i)
				for (enumerator j = 0; j < Cols(); ++j)
					assign(get(i, j), compute(i, j) - other.compute(i, j));
			return *this;
		}
		...
		AbstractMatrix & operator*=(typeB coef)
		{
			for (enumerator i = 0; i < Rows(); ++i)
				for (enumerator j = 0; j < Cols(); ++j)
					assign(get(i, j), compute(i, j) * coef);
			return *this;
		}
	...
	/// This class allows for in-place operations on submatrix of the matrix elements.
	template<typename Var>
	class AbstractSubMatrix : public AbstractMatrixReadOnly<Var>
	{
	private:
	...
	public:
		...
		/// @param first_column First column in the matrix.
		/// @param last_column Last column in the matrix.
		AbstractSubMatrix(const AbstractMatrixReadOnly<Var> & rM, enumerator first_row, enumerator last_row, enumerator first_column, enumerator last_column) : M(&rM), brow(first_row), erow(last_row), bcol(first_column), ecol(last_column)
		{}
	...
	template<typename Var>
	class AbstractBlockOfMatrix : public AbstractMatrixReadOnly<Var>
	{
	private:
		const AbstractMatrixReadOnly<Var> * M;
	...
	public:
		...
		AbstractBlockOfMatrix(const AbstractMatrixReadOnly<Var> & rM, enumerator num_rows, enumerator num_cols, enumerator offset_row, enumerator offset_col) : M(&rM), nrows(num_rows), ncols(num_cols), orow(offset_row), ocol(offset_col)
		{}
		...
		__INLINE Var compute(enumerator i, enumerator j) const
		{
			assert(i < Rows());
			assert(j < Cols());
			return !(i < orow || i >= orow + M->Rows() || j < ocol || j >= ocol + M->Cols()) ? M->compute(i - orow, j - ocol) : Var(0.0);
		}
	};
	...
	template<typename Var>
	class AbstractMatrixTranspose : public AbstractMatrixReadOnly<Var>
	{
	private:
	...
	public:
		...
		AbstractMatrixTranspose(const AbstractMatrixReadOnly<Var>& rA) : A(&rA) {}
	...
	template<typename Var>
	class AbstractMatrixConjugateTranspose : public AbstractMatrixReadOnly<Var>
	{
	private:
		const AbstractMatrixReadOnly<Var>* A;
	public:
		...
		/// @return Number of columns.
		__INLINE enumerator Cols() const { return A->Rows(); }
		AbstractMatrixConjugateTranspose(const AbstractMatrixReadOnly<Var>& rA)
			: A(&rA) {}
	...
	template<typename Var>
	class AbstractMatrixConjugate : public AbstractMatrixReadOnly<Var>
	{
	private:
		const AbstractMatrixReadOnly<Var>* A;
	public:
		...
		/// @return Number of columns.
		__INLINE enumerator Cols() const { return A->Cols(); }
		AbstractMatrixConjugate(const AbstractMatrixReadOnly<Var>& rA)
			: A(&rA) {}
	...
	template<typename VarA, typename VarB>
	class AbstractMatrixConcatRows : public AbstractMatrixReadOnly<typename Promote<VarA,VarB>::type>
	{
	private:
		const AbstractMatrixReadOnly<VarA>* A;
		const AbstractMatrixReadOnly<VarB>* B;
	public:
		...
		/// @return Number of columns.
		__INLINE enumerator Cols() const { return A->Cols(); }
		AbstractMatrixConcatRows(const AbstractMatrixReadOnly<VarA>& rA, const AbstractMatrixReadOnly<VarB>& rB)
			: A(&rA), B(&rB) 
		{
			assert(A->Cols() == B->Cols());
		}
	...
	template<typename VarA, typename VarB>
	class AbstractMatrixConcatCols : public AbstractMatrixReadOnly<typename Promote<VarA, VarB>::type>
	{
	private:
		const AbstractMatrixReadOnly<VarA>* A;
		const AbstractMatrixReadOnly<VarB>* B;
	public:
		...
		AbstractMatrixConcatCols(const AbstractMatrixReadOnly<VarA>& rA, const AbstractMatrixReadOnly<VarB>& rB)
			: A(&rA), B(&rB) {
			assert(A->Rows() == B->Rows());
		}
	...
	template<typename Var>
	class AbstractMatrixRepack : public AbstractMatrixReadOnly<Var>
	{
	private:
		const AbstractMatrixReadOnly<Var>* A;
		enumerator n, m;
	public:
		...
		/// @return Number of columns.
		__INLINE enumerator Cols() const { return m; }
		AbstractMatrixRepack(const AbstractMatrixReadOnly<Var>& rA, enumerator n, enumerator m)
	...
	template<typename VarA, typename VarB, typename VarAB>
	class MatrixMulCoef : public AbstractMatrixReadOnly<VarAB>
	{
	private:
		const AbstractMatrixReadOnly<VarA>* A;
		VarB coef;
	public:
		...
		/// @param j Column index.
		/// @return Reference to constant element.
		__INLINE VarAB compute(enumerator i, enumerator j) const { return A->compute(i, j) * coef; }
	};
	
	template<typename VarA, typename VarB, typename VarAB>
	class MatrixDivCoef : public AbstractMatrixReadOnly<VarAB>
	{
	private:
		const AbstractMatrixReadOnly<VarA>* A;
		VarB coef;
	...
	*/
	...
	AbstractMatrixReadOnly<Var>::Solve(const AbstractMatrixReadOnly<typeB> & B, int * ierr) const
	{
		...
		if( Rows() != Cols() )
		{
			AbstractMatrixTranspose<Var> At = this->Transpose(); //m by n matrix
...
INMOST::MatrixMulCoef<typeB, typeA, typename INMOST::Promote<typeB, typeA>::type>
operator *(const typeA & coef, const INMOST::AbstractMatrixReadOnly<typeB>& other)
{return INMOST::MatrixMulCoef<typeB, typeA, typename INMOST::Promote<typeB, typeA>::type>(other, coef);}
...
INMOST::MatrixMulCoef<typeB, INMOST::variable, typename INMOST::Promote<typeB, INMOST::variable>::type>
operator *(INMOST::shell_expression<A> const& coef, const INMOST::AbstractMatrixReadOnly<typeB>& other)
{return INMOST::MatrixMulCoef<typeB, INMOST::variable, typename INMOST::Promote<typeB, INMOST::variable>::type>(other, INMOST::variable(coef));}
#endif //USE_AUTODIFF
