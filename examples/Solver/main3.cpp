...
int main(int argc,char ** argv)
{
	...
	if( argc > 1 )
	{
		...
		m->SetCommunicator(MSPP_MPI_COMM_WORLD);
		...
		if( m->GetProcessorRank() == 0 ) std::cout << "Load: " << Timer()-ttt << std::endl;
#if defined(USE_PARTITIONER)
		ttt = Timer();
		Partitioner * p = new Partitioner(m);
		p->SetMethod(repartition ?  Partitioner::Parmetis : Partitioner::Inner_RCM,Partitioner::Partition);
		p->Evaluate();
		delete p;
		BARRIER
	
		if( m->GetProcessorRank() == 0 ) std::cout << "Calculation partition: " << Timer()-ttt << std::endl;
		...
		if( m->GetProcessorRank() == 0 )
		{
			...
			m->ListTagNames(tag_list);
			
			{
				std::cout << "Mesh have following tags:" << std::endl;
				for(unsigned int i = 0; i < tag_list.size(); i++)
				{
					std::cout << tag_list[i];
					Tag t = m->GetTag(tag_list[i]);
					switch(t.GetDataType())
					{
						case DATA_REAL: std::cout << " real"; break;
						case DATA_INTEGER: std::cout << " integer"; break;
						case DATA_BULK: std::cout << " bulk"; break;
						case DATA_REFERENCE: std::cout << " reference"; break;
					}
					std::cout << " components: " << t.GetSize();
					std::cout << std::endl;
				}
			}
		}
		*/
		...
		m->Save("grid.pmf");
		BARRIER
		if( m->GetProcessorRank() == 0 ) std::cout << "Save pmf: " << Timer()-ttt << std::endl;
