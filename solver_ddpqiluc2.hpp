...
#define DIAGONAL_PIVOT_TAU 0.001
#define SCHUR_DROPPING
#define SCHUR_TAU pow(tau,0.5)
#define ILUC2
#define ILUC2_TAU pow(tau,1.5)
					...
					while (i != EOL)
					{
						...
						
						{
							...
							for (INMOST_DATA_ENUM_TYPE it = U2_Address[i].first; it < U2_Address[i].last; ++it)
							{
								...
								if (LineIndeces[j] != UNDEF) //there is an entry in the list
									LineValues[j] -= l*u;
								else if (fabs(l) > tau * NuU)//add new entry
								{
									next = curr;
									while (next < j)
									{
					...
					while (i != EOL)
					{
						...
						
						{
							...
							for (INMOST_DATA_ENUM_TYPE it = L2_Address[i].first; it < L2_Address[i].last; ++it)
							{
								...
								if (LineIndeces[j] != UNDEF) //there is an entry in the list
									LineValues[j] -= l * u;
								else if (fabs(u)*NuL > tau)//add new entry
								{
									next = curr;
									while (next < j)
									{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			Emeannorm = sqrt(Emeannorm / (1.0*(wend - cend)));
			Fmeannorm = sqrt(Fmeannorm / (1.0*(wend - cend)));
