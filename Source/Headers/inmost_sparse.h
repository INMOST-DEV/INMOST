...
//#pragma pack(pop,r1)
		...
		public:
			...
			INMOST_DATA_ENUM_TYPE Nonzeros() const {INMOST_DATA_ENUM_TYPE nnz = 0; for(Matrix::const_iterator it = Begin(); it != End(); ++it) nnz += it->Size(); return nnz;}
		...
		public:
			...
			/// Retrieve/add an entry from/to linked list.
			/// @param pos Position in the list.
			INMOST_DATA_REAL_TYPE & operator [] (INMOST_DATA_ENUM_TYPE pos);
			/// Retrieve an entry from linked list.
			/// \warning
			...
			///Retrieve iterator for the first element.
			iterator Begin() {return iterator(&LinkedList);}
			///Retrieve iterator for the position beyond the last element.
