			...
			if( do_show_row(node) )
			{
				...
				if( success )
				{
					//if( min_loop.empty() || min_loop.size() >= length )
					{
						...
						Storage::real measure = compute_measure_vol(temp_loop);
						...
						if( print )
						{
							...
							for(unsigned j = 0; j < temp_loop.size(); ++j)
								std::cout << ElementTypeName(GetHandleElementType(head_column[insert_order[j]])) << ":" << GetHandleID(head_column[insert_order[j]]) << " ";
							std::cout << "measure " << measure << " len " << compute_measure(temp_loop) << std::endl;
						}
	...
	public:
		...
		void print_matrix()
		{
			...
			if( GetHandleElementType(head_column[0]) == EDGE )
			{
				...
				std::cout << "Geometry debug data" << std::endl;
				std::cout << "ASCII" << std::endl;
				std::cout << "DATASET UNSTRUCTURED_GRID" << std::endl;
				std::cout << "POINTS " << head_row.size() << " float" << std::endl;
				for (int k = 0; k < (int)head_row.size(); ++k)
				{
					Node n(mesh, head_row[k]);
					std::cout << n.Coords()[0] << " " << n.Coords()[1] << " " << n.Coords()[2] << std::endl;
				}
				std::cout << "CELLS " << head_column.size() << " " << head_column.size() * 3 << std::endl;
				for (int k = 0; k < (int)head_column.size(); ++k)
				{
					std::cout << "2";
					for (size_t j = 0; j < head_row.size(); j++)
						if (matrix[k * head_row.size() + j])
							std::cout << " " << j;
					std::cout << std::endl;
				}
				std::cout << "CELL_TYPES " << head_column.size() << std::endl;
				for (int k = 0; k < (int)head_column.size(); ++k)
					std::cout << "3" << std::endl;
			}
		}
