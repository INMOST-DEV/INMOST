        ...
        for (INMOST_DATA_ENUM_TYPE it = 0; it < overlap + 1; it++) {
            ...
            //if( !current_global_to_local.empty() )
            {
                     ...
                     k < incoming[size * 2 + rank]; k++) //receive rows that others want from me
                {
                    ...
                    GUARD_MPI(MPI_Recv(&msgsize, 1, INMOST_MPI_DATA_ENUM_TYPE, MPI_ANY_SOURCE, size + rank, comm,
                                       &stat)); //recv number of rows
                    ...
                    GUARD_MPI(MPI_Irecv(&vector_exchange_send[j], msgsize, INMOST_MPI_DATA_ENUM_TYPE, stat.MPI_SOURCE,
                                        2 * size + rank, comm, &requests[2 * vector_exchange_recv[0] + k])); //recv rows
                    j += msgsize;
                    total_send += msgsize;
                ...
                if (2 * vector_exchange_recv[0] + incoming[size * 2 + rank] > 0) GUARD_MPI(
                        MPI_Waitall(2 * vector_exchange_recv[0] + incoming[size * 2 + rank], &requests[0],
                                    MPI_STATUSES_IGNORE));
            }
            /*
			else
			...
			*/
            if (it == overlap) {
                ...
                for (std::vector<std::pair<INMOST_DATA_ENUM_TYPE, INMOST_DATA_ENUM_TYPE> >::iterator jt = current_global_to_local.begin();
                     jt != current_global_to_local.end(); ++jt)
                ...
                {
                    GUARD_MPI(MPI_Irecv(&recv_row_sizes[q], vector_exchange_recv[j + 1], INMOST_MPI_DATA_ENUM_TYPE,
                                        vector_exchange_recv[j], 3 * size + vector_exchange_recv[j], comm,
                                        &requests[k]));
                ...
                for (INMOST_DATA_ENUM_TYPE k = 0; k < vector_exchange_send[0]; k++) //send sizes of rows
                {
                    ...
                    GUARD_MPI(MPI_Isend(&send_row_sizes[q], vector_exchange_send[j + 1], INMOST_MPI_DATA_ENUM_TYPE,
                                        vector_exchange_send[j], 3 * size + rank, comm,
                                        &requests[vector_exchange_recv[0] + k])); //recv rows
                ...
                {
                    for (INMOST_DATA_ENUM_TYPE r = 0; r < vector_exchange_send[j + 1]; r++)
                        send_row_data.insert(send_row_data.end(), m[vector_exchange_send[j + 2 + r]].Begin(),
                                             m[vector_exchange_send[j + 2 + r]].End());
                    j += vector_exchange_send[j + 1] + 2;
                }


                //replace by mpi_waitsome
                if (vector_exchange_recv[0] + vector_exchange_send[0] > 0) GUARD_MPI(
                        MPI_Waitall(vector_exchange_recv[0] + vector_exchange_send[0], &requests[0],
                                    MPI_STATUSES_IGNORE));
                ...
                for (INMOST_DATA_ENUM_TYPE k = 0; k < vector_exchange_recv[0]; k++) //receive row data
                {
                    ...
                    GUARD_MPI(
                            MPI_Irecv(&recv_row_data[f], local_size, Sparse::GetRowEntryType(), vector_exchange_recv[j],
                                      4 * size + vector_exchange_recv[j], comm, &requests[k]));
                    q += vector_exchange_recv[j + 1];
                    j += vector_exchange_recv[j + 1] + 2;
                    f += local_size;
                }
                ...
                for (INMOST_DATA_ENUM_TYPE k = 0; k < vector_exchange_send[0]; k++) //receive row data
                {
                    ...
                    GUARD_MPI(
                            MPI_Isend(&send_row_data[f], local_size, Sparse::GetRowEntryType(), vector_exchange_send[j],
                                      4 * size + rank, comm, &requests[k + vector_exchange_recv[0]]));
                    q += vector_exchange_send[j + 1];
                    j += vector_exchange_send[j + 1] + 2;
                    f += local_size;
                }
                ...
                {
                    for (INMOST_DATA_ENUM_TYPE r = 0; r < vector_exchange_recv[j + 1]; r++) {
                        m[global_to_local[vector_exchange_recv[j + 2 + r]]] = Sparse::Row(&recv_row_data[f],
                                                                                          &recv_row_data[f] +
                                                                                          recv_row_sizes[q + r]);
...
#pragma omp single
#endif
        {
            ...
            for (i = 0; i < vector_exchange_recv[0]; i++) {
                ...
                GUARD_MPI(MPI_Irecv(&recv_storage[l], vector_exchange_recv[j + 1], INMOST_MPI_DATA_REAL_TYPE,
                                    vector_exchange_recv[j], vector_exchange_recv[j] * size + rank, comm,
                                    &recv_requests[i]));
            ...
            for (i = 0; i < vector_exchange_send[0]; i++) {
                ...
                GUARD_MPI(MPI_Isend(&send_storage[l], vector_exchange_send[j + 1], INMOST_MPI_DATA_REAL_TYPE,
                                    vector_exchange_send[j], rank * size + vector_exchange_send[j], comm,
                                    &send_requests[i]));
...
#else
                std::cout
                        << "Sorry, maximum product transverse condition estimation crout-ilu preconditioner is not included in this release"
                        << std::endl;
                prec = NULL;
#endif
...
#else
                std::cout << "Sorry, maximum product transverse ilu2 preconditioner is not included in this release"
                          << std::endl;
                prec = NULL;
#endif
            } else prec = NULL;
...
#pragma omp parallel
    ...
    bool Solver::Solve(Sparse::Vector &RHS, Sparse::Vector &SOL) {
        ...
        if (_pack == PETSc) {
            ...
            if (petsc_database_file == "") //set parameters if no file is set
            {
                SolverSetTolerancesPetsc(solver_data, relative_tolerance, absolute_tolerance, divergence_tolerance,
                                         maximum_iterations);
            }
        ...
        if (_pack == Trilinos_Aztec ||
            ...
            if (have_params) {
                Teuchos::updateParametersFromXmlFileAndBroadcast(specific_database_file, top_level_params.ptr(),
            ...
            if (_pack == Trilinos_Aztec) {
                ...
                ML_Epetra::MultiLevelPreconditioner *Prec = new ML_Epetra::MultiLevelPreconditioner(*Matrix, List,
                                                                                                    true);
            ...
            switch (static_cast<int>(stats[AZ_why])) {
                ...
                default: {
                    std::stringstream str;
                    str << "reason code " << static_cast<int>(stats[AZ_why])
        ...
        if (_pack == Trilinos_Belos) {
            ...
            const Teuchos::RCP<Teuchos::ParameterList> top_level_params = Teuchos::createParameterList();
            if (have_params)
                Teuchos::updateParametersFromXmlFileAndBroadcast(trilinos_belos_database_file, top_level_params.ptr(),
                                                                 Teuchos::MpiComm<int>(Teuchos::opaqueWrapper(
                                                                         RHS.GetCommunicator())));
