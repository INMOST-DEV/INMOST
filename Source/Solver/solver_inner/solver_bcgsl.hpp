...
#pragma omp parallel for
	...
	public:
		...
		bool Perturbate(Sparse::Vector & r0)
		{
#if defined(PERTURBATE_RTILDE)
			INMOST_DATA_REAL_TYPE r0_norm, length;
			INMOST_DATA_ENUM_TYPE vlocbeg, vlocend;
			INMOST_DATA_INTEGER_TYPE ivlocbeg, ivlocend;
			info->GetLocalRegion(info->GetRank(), vlocbeg, vlocend);
			ivlocbeg = vlocbeg;
			ivlocend = vlocend;
			//std::cout << "Rescue solver by perturbing orthogonal direction!" << std::endl;
...
#pragma omp parallel for
			...
			while (!halt)
			{
				...
				do
				{
					rho1 = 0;
#if defined(USE_OMP)
#pragma omp parallel for reduction(+:rho1)
#endif
					for (INMOST_DATA_INTEGER_TYPE k = ivlocbeg; k < ivlocend; ++k)
						rho1 += r0[k] * r[k];
					info->Integrate(&rho1, 1);
					if (rho1 != rho1 || 1. + fabs(rho1) == 1. || std::isinf(rho1))
					{
						//std::cout << "Asking to perturbate r_tilde since rho1 is too small " << rho1 << std::endl;
						if (!Perturbate(r0))
						{
							reason = "multiplier(0) cannot be rescued";
							halt = true;
							break;
						}
					}
...
#if defined(USE_OMP)
#pragma omp parallel for reduction(+:tempa,tempb)
#endif
				for (INMOST_DATA_INTEGER_TYPE k = ivlocbeg; k < ivlocend; k++)
				{
					tempa += t[k] * s[k];
