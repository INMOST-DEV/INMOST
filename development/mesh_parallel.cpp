...
namespace INMOST
{
	...
	void Mesh::SetCommunicator(INMOST_MPI_Comm _comm)
	{
		...
		MPI_Comm_dup(_comm,&comm);
		{
			...
			MPI_Allreduce(&t,&parallel_mesh_unique_id,1,INMOST_MPI_DATA_BIG_ENUM_TYPE,MPI_MAX,MPI_COMM_WORLD);
...
#if defined(USE_MPI2)
		int err = MPI_Alloc_mem(GetProcessorsNumber()*sizeof(unsigned)*2,MPI_INFO_NULL,&shared_space);
		if( err )
		{
			int errclass;
		...
		MPI_Win_create(shared_space,sizeof(unsigned)*GetProcessorsNumber()*2,sizeof(unsigned),MPI_INFO_NULL,comm,&window);
#endif
#endif
	}
	...
	void Mesh::ResolveShared()
	{
		...
		MPI_Allgather(&hash,1,MPI_UNSIGNED,&hashs[0],1,MPI_UNSIGNED,comm);
		...
		
		{
			...
			
			{
				...
				MPI_Allgather(&bbox[0],dim*2,INMOST_MPI_DATA_REAL_TYPE,&bboxs[0],dim*2,INMOST_MPI_DATA_REAL_TYPE,comm);
				...
				MPI_Allgather(sendsize2,2,MPI_INT,&sendsizeall[0],2,MPI_INT,comm);
				
				{
					...
					MPI_Waitall(send_reqs.size(),&send_reqs[0],MPI_STATUSES_IGNORE);
				}			
		...
		for(ElementType mask = CELL; mask >= NODE; mask = mask >> 1)
		{
			...
			for(iteratorElement it = BeginElement(mask); it != EndElement(); it++)
			{
				...
				if( it->HaveData(tag_delete) )
				{
						...
						if( estat == Ghost && std::binary_search(del_procs.begin(),del_procs.end(),mpirank) )
						{
#if defined(USE_PARALLEL_STORAGE)
							del_ghost[it->IntegerDF(tag_owner)].push_back(&*it);
#endif
	...
	void Mesh::AssignGlobalID(ElementType mask)
	{
		...
		if( m_state == Parallel )
		{
			...
			{
				int ierr = MPI_Scan(shift,shift_recv,4,INMOST_MPI_DATA_BIG_ENUM_TYPE,MPI_SUM,GetCommunicator());
	...
	void Mesh::ExchangeDataInnerEnd(std::vector<Tag> tags, parallel_storage & from, parallel_storage & to, ElementType mask, void (*Operation)(Tag tag,Element * element,INMOST_DATA_BULK_TYPE * recv_data,INMOST_DATA_ENUM_TYPE recv_size), exchange_data & storage)
	{
		...
		MPI_Waitall(storage.send_reqs.size(),&storage.send_reqs[0],MPI_STATUSES_IGNORE);
#endif
									...
									std::vector<INMOST_MPI_Request> & send_reqs, std::vector<INMOST_MPI_Request> & recv_reqs)
	{
		...
		if( parallel_strategy == 0 )
		{
			for(i = 0; i < send_bufs.size(); i++)
			{
				...
				MPI_Isend(&send_bufs[i].second[0],send_bufs[i].second.size(),MPI_PACKED,send_bufs[i].first,mpi_tag,comm,&send_reqs[i]);	
			}	
			for(i = 0; i < recv_bufs.size(); i++)
			{
				...
				MPI_Probe(MPI_ANY_SOURCE,mpi_tag,comm,&recv_stat);
				MPI_Get_count(&recv_stat,MPI_PACKED,&buffer_size);
				recv_bufs[i].second.resize(buffer_size);
				recv_bufs[i].first = recv_stat.MPI_SOURCE;
				MPI_Irecv(&recv_bufs[i].second[0],recv_bufs[i].second.size(),MPI_PACKED,recv_stat.MPI_SOURCE,mpi_tag,comm,&recv_reqs[i]);
			}
		}
		else if( parallel_strategy == 1 )
		{
			for(i = 0; i < recv_bufs.size(); i++) if( !recv_bufs[i].second.empty() )
			{
				...
				MPI_Irecv(&recv_bufs[i].second[0],recv_bufs[i].second.size(),MPI_PACKED,recv_bufs[i].first,mpi_tag,comm,&recv_reqs[i]);
			}
		...
		else if( parallel_strategy == 2 )
		{
			...
			{
				mpi_tag = ((parallel_mesh_unique_id+1)*mpisize*mpisize + (send_bufs[i].first+mpisize+rand_num))%max_tag;
				MPI_Irsend(&send_bufs[i].second[0],send_bufs[i].second.size(),MPI_PACKED,send_bufs[i].first,mpi_tag,comm,&send_reqs[i]);	
			}
		}
#endif
	}
	...
	void Mesh::PrepareReceiveInner(Prepare todo, std::vector< std::pair<int, std::vector<INMOST_DATA_BULK_TYPE> > > & send_bufs, std::vector< std::pair<int, std::vector<INMOST_DATA_BULK_TYPE> > > & recv_bufs)
	{
		...
		MPI_Win_fence(MPI_MODE_NOPRECEDE,window); //start exchange session
		for(i = 0; i < end; i++) shared_space[mpisize+i] = send_bufs[i].second.size()+1; //put data to special part of the memory
		for(i = 0; i < end; i++) MPI_Put(&shared_space[mpisize+i],1,MPI_UNSIGNED,send_bufs[i].first,mpirank,1,MPI_UNSIGNED,window); //request rdma
		MPI_Win_fence(MPI_MODE_NOSUCCEED,window); //end exchange session
		if( parallel_strategy == 0 )
		{
			unsigned num = 0;
		...
		if( todo == UnknownSize )
		{
			...
			for(i = 0; i < recv_bufs.size(); i++)
			{
				mpi_tag = ((parallel_mesh_unique_id+1)*mpisize*mpisize + (mpirank+mpisize+rand_num))%max_tag;
				//mpi_tag = parallel_mesh_unique_id*mpisize*mpisize+recv_bufs[i].first*mpisize+mpirank;
				MPI_Irecv(&send_recv_size[i],1,MPI_INT,recv_bufs[i].first,mpi_tag,comm,&reqs[i]);
			}
			for(i = 0; i < send_bufs.size(); i++)
			{
				mpi_tag = ((parallel_mesh_unique_id+1)*mpisize*mpisize + (send_bufs[i].first+mpisize+rand_num))%max_tag;
				//mpi_tag = parallel_mesh_unique_id*mpisize*mpisize+mpirank*mpisize+send_bufs[i].first;
				MPI_Isend(&send_recv_size[i+recv_bufs.size()],1,MPI_INT,send_bufs[i].first,mpi_tag,comm,&reqs[i+recv_bufs.size()]);	
			}
			MPI_Waitall(recv_bufs.size(),&reqs[0],MPI_STATUSES_IGNORE);
			for(i = 0; i < recv_bufs.size(); i++) recv_bufs[i].second.resize(send_recv_size[i]);
			MPI_Waitall(send_bufs.size(),&reqs[recv_bufs.size()],MPI_STATUSES_IGNORE);
		}
		else if( todo == UnknownSource )
		{
			...
			if( parallel_strategy == 0 )
			{
				...
				MPI_Allreduce(&recvs_at_proc_temp[0],&recvs_at_proc[0],mpisize,MPI_UNSIGNED,MPI_SUM,comm);
			...
			else if( parallel_strategy == 1 || parallel_strategy == 2 )
			{
				...
				MPI_Allgather(&size,1,MPI_INT,&allsize[0],1,MPI_INT,comm);
				...
				MPI_Allgatherv(&sends_dest_and_size[0],sends_dest_and_size.size(),MPI_UNSIGNED,&recvs_dest_and_size[0],&allsize[0],&displs[0],MPI_UNSIGNED,comm);			
				recv_bufs.clear();
				for(k = 0; k < mpisize; k++)
	...
	std::vector<int> Mesh::FinishRequests(std::vector<INMOST_MPI_Request> & recv_reqs)
	{
		...
		if( recv_reqs.empty() ) return ret; 
		MPI_Waitsome(recv_reqs.size(),&recv_reqs[0],&outcount,&ret[0],MPI_STATUSES_IGNORE);
	...
	void Mesh::ExchangeMarked(enum Action action)
	{
		...
		{ //in most cases the exchange of elements will effect only nearest neighbours, we want to detect this
			...
			MPI_Allreduce(&test,&halo_exchange,1,MPI_INT,MPI_MIN,GetCommunicator()); 
			if( halo_exchange )
			{
		...
		if( action == Migrate ) // delete packed elements
		{
		...
		if( action == Ghost ) //second round to inform owner about ghosted elements
		{
			MPI_Waitall(send_reqs.size(),&send_reqs[0],MPI_STATUSES_IGNORE);
		...
		if( action == Migrate ) //Compute new values
		{
		...
		if( action == Ghost ) 
		{
			...
			MPI_Waitall(num_wait,&send_reqs[0],MPI_STATUSES_IGNORE);
			//Probably now owner should send processors_tag data
			ExchangeData(tag_processors,CELL | FACE | EDGE | NODE);
		}
		ComputeSharedProcs();
		
		if( action == Migrate ) MPI_Waitall(send_reqs.size(),&send_reqs[0],MPI_STATUSES_IGNORE);
#endif
#if defined(USE_PARALLEL_WRITE_TIME)
		Exit();
		WriteTab(out_time) << __FUNCTION__ << " time: " << Timer() - all_time << std::endl;
