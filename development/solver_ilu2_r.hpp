		...
		for (i = mobeg; i < cbeg; i++)
		{
			for (k = F_Address[i].first; k < F_Address[i].last; ++k)
			{
				if (F_Entries[k].first >= cbeg)
				{
					...
					assert(localQ[F_Entries[k].first] >= cbeg);
				}
			}
		}
	}
...
public:
	...
	bool Initialize()
	{
		...
		INMOST_DATA_ENUM_TYPE LU_Beg, F_Beg;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			std::cout << "Fmeannorm " << Fmeannorm << " Emeannorm " << Emeannorm << std::endl;
#endif
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				for (i = cend; i < wend; ++i) //iterate over columns of F
				{
					...
					while (Li != EOL)
					{
						...
						{
							i = F_Entries[F_Address[Li].first].first;
							if (Fbeg[i] > Li)
							{
								Flist[Li] = Fbeg[i];
								Fbeg[i] = Li;
		...
		for (level = level_size.size(); level > 0; --level)
		{
			...
			for (k = cbeg; k < cend; ++k)
			{
				for (m = F_Address[k].first; m < F_Address[k].last; m++)
					temp[k] += output[F_Entries[m].first] * F_Entries[m].second;
			}
