...
namespace INMOST
{
	...
	Cell * Mesh::ReadCell(std::istream & in, std::vector<Face *> & faces, std::vector<Node *> & nodes)
	{
		...
		std::vector<Face *> sub_elements;
		std::vector<Node *> suggest_nodes;
		uconv.read_iValue(in,nlow);
		for(i = 0; i < nlow; i++)
		{
			uconv.read_iValue(in,lid);
		...
		ret = CreateCell(sub_elements,suggest_nodes);
		MIDType markers;
		uconv.read_iValue(in,markers);
		ret->SetMarkerSpace(markers);
		return ret;	
		...
		*/
		if(LFile.find(".grdecl") != std::string::npos) // this is eclipse grid
		{
			...
			}
			{
				...
				std::vector<Node *> c_nodes(8);
				std::vector< std::vector<Node *> > c_f_nodes(6);
				for(l = 0; l < 6; l++) 
					c_f_nodes[l].resize(4);
				int cbad = 0, cflat = 0;
				for(i = 0; i < nx; i++)
				{
					for(j = 0; j < ny; j++)
					{
						for(k = 0; k < nz; k++)
						{
							...
							for(l = 0; l < 6; l++)
								for(m = 0; m < 4; m++)
									c_f_nodes[l][m] = c_nodes[nodesnum[l][3-m]];
							//~ try
							{
								new_cells.push_back(CreateCell(c_f_nodes,c_nodes));
							}
							//~ catch(enum ErrorType t)
							//~ {
								//~ throw -1;
								//~ cbad++;
								//~ std::cout << "Error " << t << " ignored, cell not created, index " << i << "/" << nx << " " << j << "/" << ny << " " << k << "/" << nz << std::endl;
								//~ std::cout << "zcorn: ";
								//~ for(l = 0; l < 8; l++)
								//~ {
									//~ int i2 = i + l%2, j2 = j + (l/2)%2;
									//~ Storage::real alpha = (zcorn[i][j][k][l]-coords[i2][j2][1][2])/(coords[i2][j2][0][2]-coords[i2][j2][1][2]);
									//~ std::cout << "(" 
											  //~ << coords[i2][j2][1][0] + alpha * (coords[i2][j2][0][0] - coords[i2][j2][1][0]) << ","
											  //~ << coords[i2][j2][1][1] + alpha * (coords[i2][j2][0][1] - coords[i2][j2][1][1]) << ","
											  //~ <<  zcorn[i][j][k][l] << ") ";
								//~ }
								//~ bool flat = true;
								//~ for(l = 0; l < 4; l++)
									//~ if( fabs(zcorn[i][j][k][l]-zcorn[i][j][k][4+l]) > GetEpsilon())
										//~ flat = false;
								//~ if( flat ) 
								//~ {
									//~ std::cout << "it is flat!";
							...
							for(i = 0; i < ncells; i++)
							{
								...
								switch(ct[i])
								{
									...
									case 4: //VTK_POLY_LINE
									{
										...
										{
											e_nodes[0] = c_nodes[k];
											f_edges.push_back(CreateEdge(e_nodes));
											e_nodes[0] = c_nodes[(k+1)];
											f_edges.push_back(CreateEdge(e_nodes));
											c_faces.push_back(CreateFace(f_edges));
										}
										e_nodes.clear();
										Cell * c = CreateCell(c_faces);
										c_faces.clear();
									...
									case 6: //VTK_TRIANGLE_STRIP
									{
										...
										do
										{
											for(int q = 0; q < 3; q++)
											{
												e_nodes.push_back(d_nodes[q]);
												e_nodes.push_back(d_nodes[(q+1)%3]);
												f_edges.push_back(CreateEdge(e_nodes));
												e_nodes.clear();
											}
									...
									case 8: //VTK_PIXEL
									{
										...
										int edge[4][2] = {{0,1},{1,3},{3,2},{2,0}};
										e_nodes.resize(1);
										for(int k = 0; k < 4; k++)
										{
											e_nodes[0] = c_nodes[edge[k][0]];
											f_edges.push_back(CreateEdge(e_nodes));
											e_nodes[0] = c_nodes[edge[k][1]];
											f_edges.push_back(CreateEdge(e_nodes));
											c_faces.push_back(CreateFace(f_edges));
											f_edges.clear();
										}
									...
									case 10: //VTK_TETRA
									{
										...
										j = j + 1 + cp[j];
										if( c_nodes.size() != 4 ) throw BadFile;
										std::vector<Face *> c_faces(4);
										std::vector<Node *> f_nodes(3);
										const int nodesnum[4][3] = {{0,2,1},{0,1,3},{1,2,3},{0,3,2}};
										for(int k = 0; k < 4; k++)
										{
											for(int m = 0; m < 3; m++)
												f_nodes[m] = c_nodes[nodesnum[k][m]];//rev*2 - (2*rev-1)*m]];
											c_faces[k] = CreateFace(f_nodes);
											//if( rev ) printf("face %d reversed due to %g cell %d nf %g %g %g fc %g %g %g\n",c_faces[k]->LocalID(),dot,i,nf[0],nf[1],nf[2],fc[0],fc[1],fc[2]);
										}
										Cell * c = CreateCell(c_faces);
										newcells.push_back(c);
										break;
									...
									case 11: //VTK_VOXEL
									{
										...
										std::vector< std::vector<Node *> > c_f_nodes(6);
										const int nodesnum[6][4] = {{0,4,6,2},{1,3,7,5},{2,6,7,3},{0,1,5,4},{0,2,3,1},{4,5,7,6}};
										for(int k = 0; k < 6; k++)
											for(int m = 0; m < 4; m++)
												c_f_nodes[k].push_back(c_nodes[nodesnum[k][m]]);
										Cell * c = CreateCell(c_f_nodes);
										newcells.push_back(c);
										break;
									}
									case 12: //VTK_HEXAHEDRON
									{
										...
										if( c_nodes.size() != 8 ) throw BadFile;
										std::vector< std::vector<Node *> > c_f_nodes(6);
										const int nodesnum[6][4] = {{0,3,2,1},{4,5,6,7},{0,4,7,3},{1,2,6,5},{0,1,5,4},{3,7,6,2}};
										for(int k = 0; k < 6; k++)
											for(int m = 0; m < 4; m++)
												c_f_nodes[k].push_back(c_nodes[nodesnum[k][m]]);
										Cell * c = CreateCell(c_f_nodes);
									...
									case 13: //VTK_WEDGE
									{
										...
										const int nodesnum1[3][4] = {{0,2,5,3},{1,4,5,2},{0,3,4,1}};
										const int nodesnum2[2][3] = {{3,5,4},{0,1,2}};
										for(int k = 0; k < 3; k++)
											for(int m = 0; m < 4; m++)
												c_f_nodes[k].push_back(c_nodes[nodesnum1[k][m]]);
										for(int k = 0; k < 2; k++)
											for(int m = 0; m < 3; m++)
												c_f_nodes[k+3].push_back(c_nodes[nodesnum2[k][m]]);
										Cell * c = CreateCell(c_f_nodes);
										newcells.push_back(c);
										break;
									}
									case 14: //VTK_PYRAMID
									{
										...
										std::vector< std::vector<Node *> > c_f_nodes(5);
										const int nodesnum1[4][3] = {{0,4,3},{0,1,4},{1,2,4},{3,4,2}};
										const int nodesnum2[1][4] = {{0,3,2,1}};
										for(int k = 0; k < 4; k++)
											for(int m = 0; m < 3; m++)
												c_f_nodes[k].push_back(c_nodes[nodesnum1[k][m]]);
										for(int k = 0; k < 1; k++)
											for(int m = 0; m < 4; m++)
												c_f_nodes[k+4].push_back(c_nodes[nodesnum2[k][m]]);
										Cell * c = CreateCell(c_f_nodes);
										newcells.push_back(c);
										break;
									}
									...
									case 42: //VTK_POLYHEDRON
									{
										...
										std::vector< std::vector<int> > c_f_numbers(cp[k++]);
										for(unsigned int m = 0; m < c_f_numbers.size(); m++)
										{
											c_f_numbers[m].resize(cp[k++]);
											for(unsigned int l = 0; l < c_f_numbers[m].size(); l++)
												c_f_numbers[m][l] = cp[k++];
										}
										j = k;
										std::vector< std::vector<Node *> > c_f_nodes(c_f_numbers.size());
										for(unsigned int m = 0; m < c_f_numbers.size(); m++)
										{
											c_f_nodes[m].resize(c_f_numbers[m].size());
											for(unsigned int l = 0; l < c_f_numbers[m].size(); l++)
											{
												c_f_nodes[m][l] = newnodes[c_f_numbers[m][l]];
												newnodes[c_f_numbers[m][l]]->RemMarker(unused_marker);
											}
										}										
										newcells.push_back(CreateCell(c_f_nodes));
										break;
									}
		...
		else if(LFile.find(".pmf") != std::string::npos) //this is inner parallel/platform mesh format
		{
			...
			HeaderType token;
#if defined(USE_MPI)
			REPORT_VAL("ParallelState",m_state);
			if( m_state == Mesh::Parallel )
			{
				...
				if( parallel_file_strategy == 1 )
				{
					...
					if( mpirank == 0 ) //the alternative is to read alltogether
					{
						...
						REPORT_MPI(ierr = MPI_File_read_shared(fh,&buffer[0],buffer.size(),MPI_CHAR,&stat));
						if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);

						header << buffer;
						uconv.read_iValue(header,datanum);
						REPORT_VAL("files",datanum);
						...
						for(k = 0; k < datanum; k++) 
						{
							...
							REPORT_VAL("file_size",datasizes[k]);
						}
						...
						REPORT_VAL("pack_files",(datanum > recvsizes.size()));
						...
						else
						{
							...
							REPORT_VAL("pack_chunk",chunk);
							pos = 0;
							for(k = 0; k < recvsizes.size()-1; k++)
							{
								for(q = 0; q < chunk; q++)
									recvsizes[k] += datasizes[pos++];
								REPORT_VAL("read_file_size",recvsizes[k]);
							}
							for(k = pos; k < datanum; k++)
								recvsizes[recvsizes.size()-1] += datasizes[k];
							REPORT_VAL("read_file_size",recvsizes[recvsizes.size()-1]);
						}
					}
					...
					REPORT_VAL("local_read_file_size",recvsize);


					{
						REPORT_MPI(ierr = MPI_File_read_ordered(fh,&buffer[0],recvsize,MPI_CHAR,&stat));
						if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);
...
#endif
				{
					...
					std::vector<int> sendcnts, displs;
		...
		else if(LFile.find(".vtk") != std::string::npos) //this is legacy vtk
		{
			...
			
			{
				...
				for(unsigned int i = 0; i < tag_names.size(); i++)
				{
					...
					if( t.isDefined(CELL) && !t.isSparse(CELL) && t.GetDataType() != DATA_BULK &&
				...
				for(unsigned int i = 0; i < tags.size(); i++)
				{
					...
					if( comps > 4 ) 
					{
						//printf("Warning: vtk don't support arrays of size greater then 4 (tag name: %s)\n",tags[i].GetTagName().c_str());
						continue;
					}
			...
			}
			{
				...
				{
					Tag t = GetTag(tag_names[i]);
					if( t.isDefined(NODE) && !t.isSparse(NODE) && t.GetDataType() != DATA_BULK &&
					   t != CoordsTag() && t != SharedTag() && t != SendtoTag() && t != ProcessorsTag())
						tags.push_back(t);
				}
		...
		else if( LFile.find(".gmv") != std::string::npos) //this is gmv file
		{
			...
			for(ElementType etype = CELL; etype >= NODE; etype = etype >> 1)
			{
				...
				for(Mesh::iteratorTag t = BeginTag(); t != EndTag(); t++) 
					if( t->isDefined(etype) && t->GetSize() == 1 && !t->isSparse(etype) && t->GetTagName().substr(0,9) != "PROTECTED" )
					{
			...
			for(ElementType etype = CELL; etype >= NODE; etype = etype >> 1)
			{
				if( etype == EDGE ) continue;
				for(Mesh::iteratorTag t = BeginTag(); t != EndTag(); t++) 
					if( t->isDefined(etype) && t->GetSize() == 1 && t->isSparse(etype) )
					{
			...
			for(ElementType etype = CELL; etype >= NODE; etype = etype >> 1)
			{
				if( etype == EDGE ) continue;
				for(Mesh::iteratorTag t = BeginTag(); t != EndTag(); t++) 
					if( t->isDefined(etype) && t->GetSize() != 1 && t->GetSize() != ENUMUNDEF && !t->isSparse(etype) )
					{
						sprintf(keyword,"%s",t->GetTagName().substr(0,8).c_str());
						fwrite(keyword,1,8,file);
						switch(etype)
			...
			if( m_state == Mesh::Parallel )
			{
				...
				if( parallel_file_strategy == 1 )
				{
					...
					if( GetProcessorRank() == 0 )
					{
						...
						REPORT_VAL("processors",numprocs);
						for(k = 0; k < numprocs; k++) 
						{
							REPORT_VAL("write_file_size",datasizes[k]);
							uconv.write_iValue(header,datasizes[k]);
						}
...
#endif
				{
					...
					REPORT_MPI(ierr = MPI_Gatherv(&local_data[0],local_data.size(),MPI_CHAR,&file_contents[0],&displs[0],&recvcounts[0],MPI_CHAR,0,GetCommunicator()));
					if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);
					if( GetProcessorRank() == 0 )
					{
