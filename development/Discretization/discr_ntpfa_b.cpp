...
ntpfa_b::ntpfa_b(Automatizator * aut, Mesh * m, std::string tensor)
...
void ntpfa_b::MarkRecursive(Element * f)
{
...
void ntpfa_b::MarkRecursiveEdge(Element * f)
{
...
void ntpfa_b::get_l(Storage * e, Automatizator::stencil_pairs & out, void * user_data)
{
	Face * f = static_cast<Face *>(e);
	Element * r = f->BackCell();
	out.push_back(Automatizator::stencil_pair(r == NULL ? f : r, 1.0));
}
void ntpfa_b::get_r(Storage * e, Automatizator::stencil_pairs & out, void * user_data)
{
	Face * f = static_cast<Face *>(e);
...
expr ntpfa_b::Grad(const expr & param) const
{
...
expr ntpfa_b::Interp(ElementType etype,const expr & param) const
{
	return stencil(intrp,param);
}

void ntpfa_b::Export(std::ostream & fcout, Storage::real trans_scale, Storage::real vol_scale) const
{
