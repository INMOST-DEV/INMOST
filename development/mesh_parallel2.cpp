...
#define ENTER_FUNC() long double all_time = Timer(); WriteTab(out_time) << "<FUNCTION name=\"" << __FUNCTION__ << "\">" << std::endl; Enter();
...
namespace INMOST
{
	...
	*/
	...
	bool point_in_bbox(Storage::real * p, Storage::real bbox[6], unsigned dim)
	{
		bool ret = true;
	...
	void Mesh::ResolveShared()
	{
		...
		else
		{
			...
			else
			{
				...
				for(iteratorNode n = BeginNode(); n != EndNode(); n++) //check that node is in some bounding box
				{
				...
				time = Timer() - time;
				REPORT_STR("Prepare array of sorted nodes");
				REPORT_VAL("time",time);
				...
				for(std::vector<Node *>::iterator it = sorted_nodes.begin(); it != sorted_nodes.end(); it++)
				{
					...
					pack_real.insert(pack_real.end(),c.begin(),c.end());
				}
					
				time = Timer() - time;
				REPORT_STR("Prepare array of coordinates to pack");
				REPORT_VAL("time",time);
				...
				
				{
					...
					unsigned sendsize2[2] = {sendsize,pack_real.size()};
					REPORT_MPI(MPI_Allgather(sendsize2,2,MPI_UNSIGNED,&sendsizeall[0],2,MPI_UNSIGNED,comm));
#endif
					*/
