...
//		long double load_timer = Timer();
		...
		*/
		...
		else if(LFile.find("vtk") != std::string::npos) //this is legacy vtk
		{
			...
			//sort(&nodes[0],nodes.size(),sizeof(Node *),CompareElement,SwapElement,NULL);
			//nodes.resize(nodes.size()-empty_nodes.size());
			//empty_nodes.clear();
		...
		else if(LFile.find("pmf") != std::string::npos) //this is inner parallel/platform mesh format
		{
			...
			if( m_state == Mesh::Parallel )
			{
...
#endif
				{
					...
					ierr = MPI_Scatterv(&buffer[0],&sendcnts[0],&displs[0],MPI_CHAR,&local_buffer,recvsize,MPI_CHAR,0,GetCommunicator());
					if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);
			...
			while (!(in >> token).eof()) 
			{
				...
				else if (token == INMOST::EoMHeader)  
				{
					...
					if( m_state == Mesh::Parallel )
					{
						...
						MPI_Allgather(&size,1,INMOST_MPI_DATA_ENUM_TYPE,&procs_sizes[0],1,INMOST_MPI_DATA_ENUM_TYPE,GetCommunicator());
						...
						if( restore_state ) //we have to do something with parallel status data
						{
							...
							ierr = MPI_Allgatherv(&myprocs[0],procs_sizes[myrank],INMOST_MPI_DATA_ENUM_TYPE,&procs[0],&recvcnts[0],&displs[0],INMOST_MPI_DATA_ENUM_TYPE,GetCommunicator());
							if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);
	...
	void Mesh::Save(std::string File)
	{
		...
		else if(LFile.find("pmf") != std::string::npos) //this is inner parallel/platform mesh format
		{
				...
				for(ElementType etype = NODE; etype <= MESH; etype = etype << 1)
					if( jt->isDefined(etype) ) 
					{
						if( jt->isSparse(etype) )
						{
							...
							for(Mesh::base_iterator it = Begin(etype); it != End(); it++, q++) 
							{
								if( it->HaveData(*jt) )
								{
			...
			if( m_state == Mesh::Parallel )
			{
				...
				ierr = MPI_Gather(&datasize,1,INMOST_MPI_DATA_ENUM_TYPE,&datasizes[0],1,INMOST_MPI_DATA_ENUM_TYPE,0,GetCommunicator());
				if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);
				...
				ierr = MPI_Gatherv(&local_data[0],local_data.size(),MPI_CHAR,&file_contents[0],&displs[0],&recvcounts[0],MPI_CHAR,0,GetCommunicator());
				if( ierr != MPI_SUCCESS ) MPI_Abort(GetCommunicator(),-1);
				if( GetProcessorRank() == 0 )
				{
