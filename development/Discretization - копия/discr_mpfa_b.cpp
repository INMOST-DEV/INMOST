...
mpfa_y::mpfa_y(Automatizator * aut, Mesh * m, std::string tensor)
...
void mpfa_y::MarkRecursive(Element * f)
{
...
void mpfa_y::Init()
{
	...
	for(Mesh::iteratorElement it = m->BeginElement(EDGE | NODE); it != m->EndElement(); ++it)
	{
		...
		it->Centroid(cnt0);
		Storage::real_array acoefs = it->RealArray(interp_coefs);
		Storage::reference_array astncl = it->ReferenceArray(interp_stncl);
		adjacent<Cell> cells = it->getCells();
		Storage::real sum = 0;
		acoefs.resize(cells.size());
		astncl.resize(cells.size());
		...
		for(int k = 0; k < cells.size(); ++k) acoefs[k] /= sum;
	}
	...
	if (needBuild(f))
	{
		...
		if( !f->GetMarker(bnd_markers) )
		{
			...
			if( ret1 && ret2 )
			{
				...
				Storage::reference_array bstncl = f->ReferenceArrayDV(stncl);
				Storage::real_array bcoefs = f->RealArrayDV(coefs);
				Storage::real sum = 0;
				...
				for(int i = 0; i < 3; i++)
					for(int j = 0; j < 3; j++)
					{
						Storage::real coef = c[0][i]*c[1][j]/sum;
						if( coef > 1.0e-13 )
						{
							if( ptypes[0][i] == AVG_NONLINEAR ) t[0][i]->SetMarker(add_markers);
							if( ptypes[1][j] == AVG_NONLINEAR ) t[1][j]->SetMarker(add_markers);
							bool found1 = false, found2 = false;
							for(int k = 0; k < bstncl.size(); k++)
							{
								if( bstncl[k] == t[0][i] )
								{
									bcoefs[k] -= coef;
							...
							if( !found1 )
							{
								bstncl.push_back(t[0][i]);
								bcoefs.push_back(-coef);
							}
							if( !found2 )
							{
								bstncl.push_back(t[1][j]);
								bcoefs.push_back(coef);
							}
						}
					}
		...
		else
		{
			...
			if( ret )
			{
				...
				ret = find_triplet(r0, f, NULL, f1, +1, area1,t, c,ptypes,allowed_types);
				if( ret )
				{
					astncl.push_back(r0);
...
expr mpfa_y::Grad(const expr & param) const
{
	return stencil(scheme,param);
}

expr mpfa_y::Interp(ElementType etype,const expr & param) const
{
	return stencil(interp,param);
}

void mpfa_y::Export(std::ostream & fcout, Storage::real trans_scale, Storage::real vol_scale) const
{
