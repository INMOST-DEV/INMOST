...
class ILUC_preconditioner : public Method
{
	...
	std::vector<Solver::Row::entry> E_Entries;
	std::vector<interval<INMOST_DATA_ENUM_TYPE, Interval> *> E_Address;
	interval<INMOST_DATA_ENUM_TYPE, Solver::Row > F_Entries;
	
	//interval<INMOST_DATA_ENUM_TYPE, row_col> EF; //remember supplementary reordered E,F blocks
	interval<INMOST_DATA_ENUM_TYPE, INMOST_DATA_ENUM_TYPE> Ffirst; //remember start positions for additional blocks
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			for (k = cbeg; k < cend; ++k)
			{
				...
				for (INMOST_DATA_ENUM_TYPE jt = A_Address[invP[k]].first; jt != A_Address[invP[k]].last; ++jt)
				{
					...
					else F_Entries[i].Push(k,u); //count nonzero in column of F
				}
			...
			for (k = cend; k < wend; ++k) nnzF += F_Entries[k].Size() - Ffirst[k];
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			for (k = cend; k < wend; ++k)
			{
				...
				for (j = Ffirst[k]; j < F_Entries[k].Size(); j++)
					Fnorm += F_Entries[k].GetValue(j)*F_Entries[k].GetValue(j);
				Emeannorm += Enorm;
				Fmeannorm += Fnorm;
			}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				for (i = cend; i < wend; ++i) //iterate over columns of F
				{
					...
					Li = cbeg;
					for (j = Ffirst[i]; j < F_Entries[i].Size(); ++j) //iterate over values of i-th column of F
					{
						LineValues[F_Entries[i].GetIndex(j)] = F_Entries[i].GetValue(j);
						Li = LineIndeces[Li] = F_Entries[i].GetIndex(j) + 1;
					}
					LineIndeces[Li] = EOL;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	bool Solve(Solver::Vector & input, Solver::Vector & output)
	{
		...
		for (k = level_size[0]; k < moend; ++k) Ffirst[k] = F_Entries[k].Size();
		for (level = level_size.size(); level > 0; --level)
		{
			...
			for (k = cend; k < wend; ++k)
			{
				for (Ffirst[k]; Ffirst[k] > 0 && F_Entries[k].GetIndex(Ffirst[k] - 1) >= cbeg; --Ffirst[k])
					temp[F_Entries[k].GetIndex(Ffirst[k]-1)] += output[k] *F_Entries[k].GetValue(Ffirst[k]-1);
			}
