...
#define SCHUR_TAU 0.0//tau
//#define ILUC2
#define ILUC2_TAU tau*tau
...
public:
	...
	bool Initialize()
	{
		...
		std::vector<Solver::Row::entry> A_Entries, C_Entries, S_Entries;
		//std::vector<INMOST_DATA_ENUM_TYPE> sort_indeces;
		interval<INMOST_DATA_ENUM_TYPE, Interval> A_Address(mobeg, moend), C_Address(mobeg,moend), S_Address(mobeg,moend);
		interval<INMOST_DATA_ENUM_TYPE, INMOST_DATA_ENUM_TYPE> Ulist(mobeg, moend,UNDEF), Llist(mobeg, moend,UNDEF), Blist(mobeg,moend,UNDEF), Flist(mobeg,moend,UNDEF);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			for (k = wbeg; k < wend; ++k)
			{
				...
				if (ddmax < wgts[k].row.max / wgts[k].row.sum) ddmax = wgts[k].row.max / wgts[k].row.sum;
			}
			for (k = wbeg; k < wend; ++k)
			{
				INMOST_DATA_ENUM_TYPE max = ENUMUNDEF;
				INMOST_DATA_REAL_TYPE val = 0.0;
				for (INMOST_DATA_ENUM_TYPE it = A_Address[k].first; it != A_Address[k].last; ++it)
				{
					i = A_Entries[it].first;
					u = fabs(A_Entries[it].second);
			...
			for (k = wbeg; k < wend; ++k)
			{
				if( wgts[k].row.ind != ENUMUNDEF && (wgts[k].row.max / wgts[k].row.sum > DDPQ_TAU * ddmax) )
					//sort_wgts.push_back(wgt_coord((wgts[k].row.nnz + wgts[wgts[k].row.ind].col.nnz)* (1.0 + (wgts[wgts[k].row.ind].col.ind == k ? 1.0 : 0.0)), coord(k, wgts[k].row.ind)));
					sort_wgts.push_back(wgt_coord(wgts[k].row.max, coord(k, wgts[k].row.ind)));
			}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				for (i = cend; i < wend; ++i) if( Fbeg[i] != EOL )//iterate over columns of F
				{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void BCGS(int level, Solver::Vector & input, Solver::Vector & output)
	{
