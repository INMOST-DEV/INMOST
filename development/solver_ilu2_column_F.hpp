...
class ILUC_preconditioner : public Method
{
	...
	std::vector<interval<INMOST_DATA_ENUM_TYPE, Interval> *> E_Address, F_Address;
...
public:
	...
	bool Initialize()
	{
		...
		interval<INMOST_DATA_ENUM_TYPE, INMOST_DATA_ENUM_TYPE> Fsize(mobeg, moend);
		INMOST_DATA_ENUM_TYPE nnzF = 0;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			ReorderEF(mobeg, cbeg, wend, donePQ, localP, localQ);
			...
			//      | E   C |
			//compute B,F
			std::fill(Fsize.begin() + cend - mobeg, Fsize.begin() + wend - mobeg,0); //to colculate sizes of columns
			B_Entries.clear();
			for (k = cbeg; k < cend; ++k)
			{
				...
				for (INMOST_DATA_ENUM_TYPE jt = A_Address[invP[k]].first; jt != A_Address[invP[k]].last; ++jt)
				{
					...
					else Fsize[i]++; //count nonzero in column of F
				}
				B_Address[k].last = B_Entries.size();
				std::sort(B_Entries.begin()+B_Address[k].first, B_Entries.end());
			}
			//Establish the columns of F
			nnzF = Fsize[cend];
			F_Address.back()->at(cend).first = F_Entries.size();
			F_Address.back()->at(cend).last = F_Address.back()->at(cend).first + Fsize[cend];
			for (k = cend + 1; k < wend; ++k)
			{
				F_Address.back()->at(k).first = F_Address.back()->at(k - 1).last;
				F_Address.back()->at(k).last = F_Address.back()->at(k).first + Fsize[k];
				nnzF += Fsize[k];
			}
			//preallocate memory
			F_Entries.resize(F_Entries.size() + nnzF);
			//now fill array
			for (k = cbeg; k < cend; ++k)
			{
				for (INMOST_DATA_ENUM_TYPE jt = A_Address[invP[k]].first; jt != A_Address[invP[k]].last; ++jt)
				{
					...
					if (i >= cend)
					{
						F_Entries[F_Address.back()->at(i).last - Fsize[i]].first = k;
						F_Entries[F_Address.back()->at(i).last - Fsize[i]].second = u;
						Fsize[i]--;
					}
				}
			}
			//Setup column addressing, in descending order to keep list ordered
			for (k = cend; k > cbeg; --k)
			{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			{
				...
				{
					std::cout << "No diagonal value! " << cbeg << std::endl;
					LU_Diag[cbeg] = 0.0;
				}
				...
				for (INMOST_DATA_ENUM_TYPE r = B_Address[k].first + (B_Entries[B_Address[cbeg].first].first == k ? 1 : 0); r != B_Address[k].last; ++r) 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			for (k = cend; k < wend; ++k)
			{
				...
				for (j = F_Address.back()->at(k).first; j < F_Address.back()->at(k).last; j++)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				for (i = cend; i < wend; ++i) //iterate over columns of F
				{
					...
					Li = cbeg;
					for (j = F_Address.back()->at(i).first; j < F_Address.back()->at(i).last; ++j) //iterate over values of i-th column of F
					{
						LineValues[F_Entries[j].first] = F_Entries[j].second;
						Li = LineIndeces[Li] = F_Entries[j].first + 1;
					}
					LineIndeces[Li] = EOL;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	bool Finalize()
	{
		...
		for (INMOST_DATA_ENUM_TYPE k = 0; k < E_Address.size(); k++)
			delete E_Address[k];
		E_Address.clear();
		for (INMOST_DATA_ENUM_TYPE k = 0; k < F_Address.size(); k++)
			delete F_Address[k];
		F_Address.clear();
		return true;
	}
	bool Solve(Solver::Vector & input, Solver::Vector & output)
	{
		assert(&input != &output);
		//
		INMOST_DATA_ENUM_TYPE k, m, level, mobeg, moend, wbeg, wend, cbeg, cend, vbeg, vend;
		info->GetOverlapRegion(info->GetRank(), mobeg, moend);
		info->GetVectorRegion(vbeg, vend);
		...
		for (level = 0; level < level_size.size(); ++level)
		{
			//current B block interval
			cbeg = wbeg;
			cend = wbeg + level_size[level];
			// step 1) obtain ~f
			//apply B^{-1} on f
			//Solve with L first
			...
			for (k = cend; k < wend; ++k)
			{
				for (m = E_Address[level]->at(k).first; m < E_Address[level]->at(k).last; ++m)
					output[k] -= output[E_Entries[m].first] * E_Entries[m].second;
			}
			//goto the next level
			wbeg = cend;
		}
		...
		for (level = level_size.size(); level > 0; --level)
		{
			wbeg -= level_size[level-1];
			cbeg = wbeg;
			//calculate Fy, iterate over ~g, write to unused input vector
			std::fill(temp.begin() + cbeg - mobeg, temp.begin() + cend - mobeg,0.0);
			for (k = cend; k < wend; ++k)
			{
				for (m = F_Address[level-1]->at(k).first; m < F_Address[level-1]->at(k).last; m++)
					temp[F_Entries[m].first] += output[k] *F_Entries[m].second;
			}
